...
%% Load and declare
% clear all; close all;
function [FPyrsout,FIntsout,rotationout,forwardvelout,corrspyrout,corrsintout,...
    distpyrsout,distintsout,cpppyr,cppint,yout,rewout,corrsoutrunpyr,corrsoutrunint,...
    corrsoutwoezpyr,corrsoutwoezint,mfballout,mtballout,mtaballout,mvrout,mvraout,...
    mPRTAoutpyr,mPRTAoutint,dFcorrpyr,dFcorrint]=...
    MixedNeuronTestCPP(varargin)
set(0,'DefaultFigureColormap',jet)
dFcorrpyr=[];
dFcorrint=[];
saveDir='F:\MA Data\Mixed\';
if nargin<1
    Fs=input('Fs? ');
...
[F,ybin,forward,rotation,cpp,masks,Fraw,rew]=align_running(paths,names,planes);
