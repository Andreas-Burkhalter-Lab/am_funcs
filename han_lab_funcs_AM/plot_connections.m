function [distall,corrall]=plot_connections(Fall,masks,cpp,varargin)
if nargin>3
    saveDir=varargin{1};
    MouseID=varargin{2};
    if nargin>5
...
savefig([saveDir,'\',MouseID,'\', MouseID, 'CorrMat',env,'.fig']);
...
saveas(gca,[saveDir,'\',MouseID,'\', MouseID, 'Bar Corr with Distances',env, '.jpg']);
figure
scatter(distall,corrall)
xlabel('Distance')
ylabel('Correlation')
saveas(gca,[saveDir,'\',MouseID,'\', MouseID, 'Scatter Corr with Distances',env, '.jpg']);

if exist('corrRow','var')
    import mlreportgen.dom.*;
    append(corrRow,TableEntry(Image([saveDir,'\',MouseID,'\',  MouseID, 'CorrMat',env,'.jpg'])));
    append(scatterRow,TableEntry(Image([saveDir,'\',MouseID,'\', MouseID, 'Scatter Corr with Distances',env,'.jpg'])));
    append(barRow,TableEntry(Image([saveDir,'\',MouseID,'\', MouseID, 'Bar Corr with Distances',env, '.jpg'])));
