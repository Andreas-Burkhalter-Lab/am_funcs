function [mixedsig, mixedfilters, CovEvals, covtrace, movm, ...
    movtm] = CellsortPCA_MA(fn, flims, nPCs, dsamp, inputdir, badframes,sizeMov)
...
% if nargin<5 || isempty(outputdir)
    outputdir = [inputdir,'/cellsort_preprocessed_data/'];
% end
if nargin<6
    badframes = [];
end
if isempty(dir(outputdir))
    mkdir(outputdir, '/cellsort_preprocessed_data/')
end
if outputdir(end)~='/';
    outputdir = [outputdir, '/'];
end
...
%     [mixedsig] = reload_moviedata(nt, mov, mixedfilters, CovEvals);
    [mixedsig] = reload_moviedata(npix, mov, mixedfilters, CovEvals); %MWA
end
...
%                 jj = useframes(jjind);
%                 mov(:,:,jjind) = imread(fn,jj,'Info',info);
%                 if mod(jjind,500)==1
...
%             for jjind=1:length(useframes)
%                 jj = useframes(jjind);
%                 mov(:,:,jjind) = imresize( imread(fn,jj,'Info',info), 1/dsamp_space, 'bilinear' );
%                 if mod(jjind,500)==1
%                     fprintf(' Read frame %4.0f out of %4.0f; ', jjind, nt)
%                     toc
%                 end
...
%             clear chone_corr
%         end
%         mov=double(video(:,:,useframes));
%         clear video

        if dsamp_space > 1
            %Do this twice to downsample by height and width.
           mov=downsample(permute(mov, [2 1 3]), dsamp_space);
           mov=downsample(permute(mov, [2 1 3]), dsamp_space);
        end
...
%         mov = mov ./ (movm * ones(1,nt)) - 1; % Compute Delta F/F
        mov = bsxfun(@rdivide,mov,movm)-1; %MWA maybe a little faster without making a second ludicrously big matrix
...
%         movtm = mean(mov,2); % Average over space
        movtm=mean(mov,1); %actually average over space MWA
        clear movmzeros
...
%         mov = mov ./ (movm * ones(1,nt) - 1);
        mov=single(bsxfun(@rdivide,double(mov),double(movm * ones(1,nt) - 1)));
        mov(movmzero, :) = 0;
    ...
    function [mixedsig, CovEvals, percentvar] = cellsort_svd(covmat, nPCs, nt, npix)
        ...
        if nPCs<size(covmat,1)
            [mixedsig, CovEvals] = eigs(double(covmat), nPCs, 'LM', opts);  % pca_mixedsig are the temporal signals, mixedsig
        else
