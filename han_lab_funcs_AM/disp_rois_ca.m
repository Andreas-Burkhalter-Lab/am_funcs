function disp_rois_ca(varargin)
if nargin==0
...
for p=1:nplanes
    ...
    masks=zeros(size(A_or,2),size(Cn,1),size(Cn,2));
    for ii=1:size(A_or,2)
        masks(ii,:,:)=reshape(A_or(:,ii),size(Cn,1),size(Cn,2));
    end
    ...
    saveas(gcf,[pathsF{p},nameF{p}(1:(end-4)),'RoisCalled_ca.jpg'])
    ...
    saveas(gcf,[pathsF{p},nameF{p}(1:(end-4)),'AddRoisCalled_ca.jpg'])
    figure('units','normalized', 'Position', [.01 .05 .98 .87]);
    subplot(1,2,1)
    ...
    prev_sum=prev_sum+size(masks,1);
    subplot(1,2,2)
    plot(bsxfun(@plus,bsxfun(@rdivide,Fca,max(Fca,2)),1:size(Fca,2)))
    saveas(gcf,[pathsF{p},nameF{p}(1:(end-4)),'RoiActivityCalled_ca.jpg'])
