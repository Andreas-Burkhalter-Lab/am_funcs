%modification of "abf_analysis_sidney_CPP_15min"
%rooms in 1st 15min and total session
close all
clear all;
...
%convert ypos (raw) to "cm"
data(:,2)=data(:,2)-min(data(:,2));  
data(:,2)=data(:,2)/max(data(:,2));
data(:,2)=data(:,2)*180;
timebin=15*60*1000;%15min at 1ms sampling frequency
...
%below for entire session
subplot(1,2,2)
 plot(data(:,2));
 title('Position in Entire Session')
...
rmA_1=find(datamat(:,1,2)<60);
results(1,1)=size(rmA_1,1)/1000;  %time spent in rmA in 1st 15min in s
rmB_1=find(datamat(:,1,2)>120);
results(1,2)=size(rmB_1,1)/1000;%time spent in rmB in 1st 15min in s
% rmA_2=find(datamat(:,2,2)<60);
% results(2,1)=size(rmA_2,1)/1000;%time spent in rmA in 2nd 15min in s
% rmB_2=find(datamat(:,2,2)>120);
% results(2,2)=size(rmB_2,1)/1000;%time spent in rmB in 2nd 15min in s

%results for entire session
rmA_2=find(data(:,2)<60);
results(2,1)=size(rmA_2,1)/1000;%time spent in rmA total in s
rmB_2=find(data(:,2)>120);
results(2,2)=size(rmB_2,1)/1000;%time spent in rmB total in s
