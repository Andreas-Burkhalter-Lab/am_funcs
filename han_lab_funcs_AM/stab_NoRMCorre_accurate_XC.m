...
for f=1:num_files
    [files{f},paths{f}]=uigetfile('*XC*.mat','pick your files');
...
%             'output_type','h5','h5_filename',[name(1:end-4),'_nr.h5']);
        options_nonrigid = NoRMCorreSetParms('d1',FOV(1),'d2',FOV(2),'grid_size',[128,128],... %grid_size,[128,128]
            'overlap_pre',64,'mot_uf',2,'bin_width',1000,'max_shift',6,'max_dev',8,'us_fac',25,...
            'output_type','h5','h5_filename',[name(1:end-4),'_nr.h5']);
        [M,shifts,template] = normcorre_batch_XC(name,options_nonrigid,template);
        save(strrep(name,'XC','shifts_NC'),'shifts','-v7.3');           % save shifts of each file at the respective subfolder
