function coords2D = transformCurvedScreen_mwa(coords3D)
...
%viewAngle = 70*pi/180;
viewAngle= 110*pi/180;%samsung
mviewAngle =  60*pi/180;%samsung
cviewAngle = 90*pi/180;
...
% 
% theta2=theta;
% theta2((theta2-viewAngle)>linview) = ...
%     (mviewAngle-linview)/(viewAngle-linview)*theta2((theta2-viewAngle)>linview)...
%     +(1-(mviewAngle-linview)/(cviewAngle-linview))*(cviewAngle+linview);
% theta2((theta2-viewAngle)<-linview)= ...
%     (mviewAngle-linview)/(viewAngle-linview)*theta2((theta2-viewAngle)<-linview)...
%     +(1-(mviewAngle-linview)/(cviewAngle-linview))*(cviewAngle-linview);

% theta2=4*atan(theta-viewAngle)+viewAngle;

% %nonlinear transform
% 
pow=1.7;
theta_neg=find(theta<viewAngle);
theta2 = abs(theta-viewAngle).^(1/pow);
theta2(theta_neg) = (-1)*theta2(theta_neg);
theta2=theta2+viewAngle^(1/pow);
theta2=theta2*(viewAngle^((pow-1)/(pow)));

m=6;
n=1;
theta2=(m*theta+n*theta2)/(m+n);
     plot(theta2)
% calculate screen x for each point
coords2D(1,:) = aspectRatio./(mviewAngle).*theta2 - aspectRatio;
