...
% if isfield(vr, 'numRewards')%test if there is a field in struct
%     disp(['The animal received ' num2str(vr.numRewards) ' test.']);
% % display the total number of rew, if running extra reward track
% %disp(['The animal received ' num2str(vr.water) ' total rewards.']);
% end
% Run termination code

if isfield (vr,'Aelapse')   %EBH
    disp([ num2str(vr.Atime) ' s and ' num2str(vr.Arewards) ' rew in A.  ' num2str(vr.Btime) ...
    ' s and '  num2str(vr.Brewards) ' rew in B']);
end
