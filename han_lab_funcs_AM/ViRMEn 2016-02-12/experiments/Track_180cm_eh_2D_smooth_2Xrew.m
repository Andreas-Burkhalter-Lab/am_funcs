function code = Track_180cm_eh_2D_smooth_2Xrew
...
vr.topTarget = 171;%from "linearTrackTwoDirections"
vr.bottomTarget = 9;%new term
...
%original again
    ...
    vr.isReward = 0;
if vr.position(2) > 172 % test if the animal is at the end of the track (y > 200)
    vr.dp(4) = vr.dp(4)*4; % set the animal’s y position to 0
%     vr.dp(4) = vr.dp(4)*2; % set the animal’s y position to 0
    %vr.dp(:) = 0; % prevent any additional movement during teleportation
end

if vr.position(2) < 8 % test if the animal is at the end of the track (y > 200)
    vr.dp(4) = vr.dp(4)*4; % set the animal’s y position to 0
%     vr.dp(4) = vr.dp(4)*2; % set the animal’s y position to 0
    %vr.dp(:) = 0; % prevent any additional movement during teleportation
end
