function code = bigCircularArenaTargetChase
...
vr.radiusPower = eval(vr.exper.variables.radiusPower);
...
if vr.clockwiseness ~= 0   
    if norm(vr.position(1:2)) < vr.minTriggerRadius
        ...
        isMissed = false;
        lst = vr.worlds{1}.objects.triangles(vr.worlds{1}.objects.indices.targetCylinder,:);
        vr.worlds{1}.surface.visible(lst(1):lst(2)) = false;
    end
    if vr.rememberMiss && norm(vr.position(1:2)) >= vr.minTriggerRadius
        ...
        vr.rememberMiss = false;
        lst = vr.worlds{1}.objects.triangles(vr.worlds{1}.objects.indices.targetCylinder,:);
        vr.worlds{1}.surface.visible(lst(1):lst(2)) = true;
    end
end
...
if isReward || isMissed
    ...
    while (norm(vr.targetPosition) < vr.minTriggerRadius && norm(vr.targetPosition - vr.position(1:2)) < vr.minStartDistanceInside) || ...
        ...
        if vr.clockwiseness == 0
            ang = rand*2*pi;
            minR2 = 0^vr.radiusPower;
            maxR2 = (vr.floorWidth/2)^vr.radiusPower;
            ...
            minR2 = max(vr.minTargetRadius,currR-vr.maxTargetRadiusShift(2))^vr.radiusPower;
            maxR2 = min(vr.floorWidth/2,currR+vr.maxTargetRadiusShift(1))^vr.radiusPower;
            r = (minR2+rand*(maxR2-minR2))^(1/vr.radiusPower);
        end
        vr.targetPosition = [r*cos(ang) r*sin(ang)];
    end
