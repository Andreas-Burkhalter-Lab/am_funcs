function code = Track_180cm_eh_test1_2Xrew_world_switchRotGain_test_speed
%modified for testing ball speed
%for testing forward, shut off rot dp
%for testing rot,set rot = for
%change ball gains so 2.8 rot = 1.8m
%turn off rotation track logic
...
vr.dp(2) = vr.dp(4);%turn rotation dp into for dp
vr.dp(4) = 0;%set rotation dp =0
if vr.position(2) < 3 % test if the animal is at the end of the track (y > 200)
    %vr.dp(4) = vr.dp(4)*3.5; % set the animal’s y position to 0
    vr.dp(2) = 0; % set the animal’s y position to 0
    %vr.dp(:) = 0; % prevent any additional movement during teleportation
end
if vr.position(2) > 176
    vr.dp(2) = 0;
