...
for f=1:num_files
    [files{f},paths{f}]=uigetfile('*plane*.mat','pick your files');
...
% for i=1
    name = files{i};
    if non_rigid
%         options_nonrigid = NoRMCorreSetParms('d1',FOV(1),'d2',FOV(2),'grid_size',[64,64],... %grid_size,[128,128]
%             'overlap_pre',32,'mot_uf',6,'bin_width',100,'max_shift',24,'max_dev',8,'us_fac',50,...
%             'output_type','h5','h5_filename',[name(1:end-4),'_nr.h5']);
        ...
        save(strrep(name,'plane','nrigshifts_NC'),'shifts','-v7.3');           % save shifts of each file at the respective subfolder
