...
[feature_count.up, ~] = calculate_feature_count(Fdir.up,pos(run.up),ceil(max(pos)));
...
prior_prob.up = histcounts(floor(pos(run.up)),1:binsize);
prior_prob.up = prior_prob.up/sum(prior_prob.up);
prior_prob.down = histcounts(floor(pos(run.down)),1:binsize);
