...
figure('units','normalized', 'Position', [.01 .05 .98 .87]);
for n=1:nfiles
    for ii=1:(length(plotcells)+1)
        if ii<(length(plotcells)+1)

        (n-1)*(length(plotcells)+1)+ii
        subplot(nfiles,length(plotcells)+1,(n-1)*(length(plotcells)+1)+ii)
        hold on
        cellF=Ffile{n}(:,plotcells(ii));
        ...
        elseif ii==(length(plotcells)+1)

            subplot(nfiles,length(plotcells)+1,(n-1)*(length(plotcells)+1)+ii)
            hold on
            cellF=Ffile{n};
            mcell(1)=mean(mean(cellF(envinds{n}{1},:)));
            mcell(2)=mean(mean(cellF(envinds{n}{2},:)));
            mcell(3)=mean(mean(cellF((envinds{n}{2}+1):end,:)));
