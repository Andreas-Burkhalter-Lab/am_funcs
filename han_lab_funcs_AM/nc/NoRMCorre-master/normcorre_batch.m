...
%         filetype = 'mat'; %mwa
        ...
        sizY = size(Y,'chone'); %added the chone part mwa
        T = sizY(end);
...
%         if nd == 2; Y_temp = single(Y.Y(:,:,1:init_batch)); elseif nd == 3; Y_temp = single(Y.Y(:,:,:,1:init_batch)); end
        if nd == 2; Y_temp = single(Y.chone(:,:,1:init_batch)); elseif nd == 3; Y_temp = single(Y.chone(:,:,:,1:init_batch)); end%mwa
    case 'mat'
...
%                 if nd == 2; Ytm = single(Y.Y(:,:,t:min(t+bin_width-1,T))); end
                if nd == 2; Ytm = single(Y.chone(:,:,t:min(t+bin_width-1,T))); end %mwa
