function loadVideoH5(filepath,filename,plane_num)
...
%     tic
    chone=uint16(newmov(:,:,j:plane_num:end));
...
%     tic
    currfile=strcat(stripped_filename,'.h5');
%     currfile=sprintf(currfile,j);
    currfilename=[filepath currfile];
%     if ~exist
    h5create(currfilename, ['/DS',num2str(j)], size(chone));
    
    h5write(currfilename, ['/DS',num2str(j)], (chone));
