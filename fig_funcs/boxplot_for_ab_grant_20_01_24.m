...
axis_font_size = 13; 
resolution_dpi = 300; 
savename = 'pakan_loc_mod_boxplot';
...
ops.meets_criteria = ops.meets_criteria & any(roitable.quantile == [1 6],2); %%% only analyze specific quantiles
...
if ops.show_histograms
    for ii = 1:height(sorted_vals)
        histogram(sorted_vals{ii,ops.varname}{:})
        hold on
    end
hold off
end
...
set(gca,'LooseInset',get(gca,'TightInset')+[0 .01 0.005 0]) % crop borders
print(gcf, [savename,'.tif'], '-dtiffn',['-r',num2str(resolution_dpi)])
