...
%%% updated 2020/12/3 on thermaltake
    ...
    if strcmp(pars.threshMode,'intensityQuantiles') % if we need to identify a quantile for the nonroi spot
        ...
        for iblob = 1:max(max(nan_lm))
            ...
            if any(any(this_blob_im & ~roi)) % if this blob was marked as outside the roi
                ...
                bordering_quant_vals = quant_levels_img(blob_with_outer_bnds_inds);
                if any(bordering_quant_vals > 0) % normal case - we can assign a nonzero quant
                    mode_bordering_quantile = mode(bordering_quant_vals(bordering_quant_vals>0)); % finding most frequent quantile touching this non-roi blob
                ...
                elseif ~any(bordering_quant_vals > 0) % rare case where the blob is entirely surrounded by quant==0
                    mode_bordering_quantile = 0; %% asign as outside of ROI
                end
