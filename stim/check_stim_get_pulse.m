...
%%% last updated 18/11/1 on thermaltake

function pulsepars_out = check_stim_get_pulse(stimpars,pulsepars_in,daq_sess,commonvars)
...
pulsepars_out.stimon_signal = pulsepars_in.Vstimon * ones(pulsepars.stimon_dur_scans,1); % signal to send at stim onset
pulsepars_out.stimoff_signal = pulsepars_in.Vstimoff * ones(pulsepars.stimoff_dur_scans,1); % signal to send at stim offset
    
%%%% old parameter checks, need to be fixed
