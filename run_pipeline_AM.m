function  run_pipeline_AM(db, ops0)
%%%% edited by AM 2020/3/21 to reduce NimgFirstRegistration if it's greater than or equal to number of frames in image stack
...
%%%% added by AM - reduce NimgFirstRegistration if it's greater than or equal to number of frames in image stack
if length(ops.fsroot{1}) < 2 % low frame number should not be a problem if there's more than 1 input image stack
    nframes_in_stack = length(imfinfo(ops.fsroot{1}.name)); % number of images in stack
    ops.NimgFirstRegistration = min([ops.NimgFirstRegistration, floor(nframes_in_stack/ops.nplanes)-1]); % reduce NimgFirstRegistration if necessary
end
%%%%
