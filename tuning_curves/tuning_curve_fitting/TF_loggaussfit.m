...
%	Params are as follows: Bg (q1)=base rate, A (q2) = scale factor (aka amplitude), vn (q3), sigma (q4), v0 (q5)
...
%%%% for plotting: fn = @(pars,qdata)pars(2).*exp(-1./2./pars(4).^2 .* (log((qdata+pars(5))./(pars(3)+pars(5)))).^2) + pars(1)
...
OPTIONS = optimset('Display','off','MaxIter', 5000); %Ji replaced front 2 lines by this line10/5/2015
N_reps = 10;
wiggle = 0.6;
testpars = []; err=[];
for j=1:N_reps
