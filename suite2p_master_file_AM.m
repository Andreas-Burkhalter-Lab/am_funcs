...
% addpath(genpath('C:\Users\Admin\Desktop\BigData\Marius Pachitariu\Suite2P-master')) 
% addpath(genpath('C:\Users\Admin\Desktop\BigData\Marius Pachitariu\OASIS-master')) 

% mainfilepath\mouse_name\date\expts

%%% edited by AM on thermaltake 2020/1/21

i = 0;

i = i+1;
db(i).mouse_name    = '19089'; %First folder after Root
db(i).date          = '2020-01-20'; %Second folder after Root
db(i).nchannels     = 1;
db(i).gchannel      = 1; 
db(i).nplanes       = 2; %Number of planes
db(i).comments      = '';
db(i).diameter      = 12;

ops0.toolbox_path = 'C:\Users\Burkhalter Lab\Documents\matlab_functions\han_lab_funcs_AM\Suite2P-master';
...
ops0.temp_tiff              = 'C:\TempData\temp.tif'; % copies each remote tiff locally first, into this file
ops0.RegFileRoot            = 'C:\TempData\';  % location for binary file
...
ops0.RegFileTiffLocation    = 'F:\';  % leave empty to NOT save registered tiffs (slow)
...
ops0.NimgFirstRegistration  = 500; % number of images to include in the first registration pass.... will need to reduce if recording is shorter than this number of frames
...
ops0.NavgFramesSVD          = 2000; % how many (binned) timepoints to do the SVD based on
...
ops0.imageRate              = 3.3;   % imaging rate (cumulative over planes!). Approximate, for initialization of deconvolution kernel.
ops0.sensorTau              = .5; % decay half-life (or timescale). Approximate, for initialization of deconvolution kernel.
