...
%%%% modified by AM 18-10-08 on thermaltake
...
is_sbx = false(num_files,1);
for f=1:num_files
    [name,paths{f}]=uigetfile('*.sbx;*.tif','pick your files');
    is_sbx(f) = strcmp('.sbx',name(end-3:end));
    files{f}=name(1:end-4);
    file_ext{f} = name(end-3:end);
    if is_sbx(f)
...
%     loadVideoTiff(paths{f},files{f},num_planes(f),Fs);
    loadVideoTiffNoSplit_AM(paths{f},files{f},scan_type{f},is_sbx(f),file_ext{f});
end
