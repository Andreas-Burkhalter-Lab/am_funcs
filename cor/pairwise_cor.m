...
%%%%% updated 2020/8/31 on thermaltake
...
pars.show_waitbar = field_default(pars,'show_waitbar',1); 
...
% tabulate roi pairs
tuningdat.tuning.rf_area_sqdeg = pi * tuningdat.tuning.rf_size_yx_deg(:,1)/2 .* tuningdat.tuning.rf_size_yx_deg(:,2)/2; % sq deg; estimate as ellipse
icombo = 0; % counter to go down cortable rows
if pars.show_waitbar; wbar = waitbar(icombo/ncombos,'Computing dff correlations...'); end
for roi1 = 1:nrois
    for roi2 = roi1+1:nrois % pair roi1 with every roi it hasn't yet been paired with
        ...
        cortable.sf_sgnf(icombo,1:2) = [tuningdat.tuning.sf_sgnf(roi1), tuningdat.tuning.sf_sgnf(roi2)]; 
        cortable.sf_anovap(icombo,1:2) = [tuningdat.tuning.sf_anovap(roi1), tuningdat.tuning.sf_anovap(roi2)];
        cortable.tf_sgnf(icombo,1:2) = [tuningdat.tuning.tf_sgnf(roi1), tuningdat.tuning.tf_sgnf(roi2)]; 
        cortable.tf_anovap(icombo,1:2) = [tuningdat.tuning.tf_anovap(roi1), tuningdat.tuning.tf_anovap(roi2)];
        cortable.orient_sgnf(icombo,1:2) = [tuningdat.tuning.orient_sgnf(roi1), tuningdat.tuning.orient_sgnf(roi2)]; 
        cortable.orient_anovap(icombo,1:2) = [tuningdat.tuning.orient_anovap(roi1), tuningdat.tuning.orient_anovap(roi2)]; 
        cortable.rf_sgnf(icombo,1:2) = [tuningdat.tuning.rf_sgnf(roi1), tuningdat.tuning.rf_sgnf(roi2)]; 
        cortable.rf_sgnf(icombo,1:2) = [tuningdat.tuning.rf_sgnf(roi1), tuningdat.tuning.rf_sgnf(roi2)]; 
        cortable.locm_sgnf(icombo,1:2) = [tuningdat.tuning.locm_cor_p(roi1) < .05, tuningdat.tuning.locm_cor_p(roi2) < .05]; 
        cortable.locm_cor_r(icombo,1:2) = [tuningdat.tuning.locm_cor_r(roi1), tuningdat.tuning.locm_cor_r(roi2)]; 
        cortable.stim_on_rf(icombo,1:2) = [tuningdat.tuning.stim_on_rf(roi1), tuningdat.tuning.stim_on_rf(roi2)]; 
        cortable.rf_height_deg(icombo,1:2) = [tuningdat.tuning.rf_size_yx_deg(roi1,1), tuningdat.tuning.rf_size_yx_deg(roi2,1)];
        cortable.rf_width_deg(icombo,1:2) = [tuningdat.tuning.rf_size_yx_deg(roi1,2), tuningdat.tuning.rf_size_yx_deg(roi2,2)];
        cortable.rf_area_sqdeg(icombo,1:2) = [tuningdat.tuning.rf_area_sqdeg(roi1), tuningdat.tuning.rf_area_sqdeg(roi2)];
        if any(ismember(tuningdat.tuning.Properties.VariableNames,'rspv'))
            cortable.rspv(icombo,1:2) = [tuningdat.tuning.rspv(roi1), tuningdat.tuning.rspv(roi2)]; 
            cortable.rspv_pval(icombo,1:2) = [tuningdat.tuning.rspv_pval(roi1), tuningdat.tuning.rspv_pval(roi2)]; 
        else
            cortable.rspv(icombo,1:2) = [false, false];
            cortable.rspv_pval(icombo,1:2) = [NaN, NaN]; 
        end
