function [  ] = warped_stim_generate_thermaltake(  )
...
prerend.sf_fixed = 0.05; % cpd
prerend.tf_fixed = 2; % hz
prerend.orient_fixed = 270; % deg
prerend.sf_minmax = [0.05 0.05];      %% range of annulus spatial frequencies in cycles per degree (~[0.01 1.6] in Gao)
prerend.tf_minmax = [0.1 13];       %% range of annulus temporal frequencies in hz (~[0.1 13] in Gao fig 1)
prerend.square_wave = 0; % if true, use square wave rather than sine wave gratings
prerend.outer_amp = 1; % deviation from the midpoint of the grating brightness (max possible = 1); affects contrast, not mean brightness of grating
...
prerend.n_tfs = 0;      % number of values of annulus temporal frequency to present
prerend.n_orients = 0; 
...
prerend.tfsIfiMultiples = 1; % set all tfs >= 1/stimdur to integer values of flips/cycle to reduce # of gratings
