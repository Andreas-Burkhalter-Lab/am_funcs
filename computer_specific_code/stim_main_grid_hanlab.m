...
function stim_main_grid_hanlab()

%% Setup
stimpars.manual_grid_center = 0;
    stimpars.grid_center = []; % x and y coords of grid center; if blank, center will be at center of stim screen; overridden by stimpars.manual_grid_center = true
stimpars.keypress_between_trials = 0; % get keyboard input between every trial
stimpars.random_order = 1; % if true, show stim in random order; otherwise, show by rows then columns
stimpars.repetitions = 9; %%  number of full iterations of grid presentation
stimpars.rows =6;       %% number of rows of stimuli in the grid
stimpars.columns = 4;    %% number of columns of stimuli in the grid
stimpars.h_spacing = 15; %% horizontal distance in degrees between stim centers (not specified in Gao)
stimpars.v_spacing = 15; %% vertical distance in degrees between stim centers (not specified in Gao)
stimpars.diam = 22; %% diameter of stimuli in degrees; divide by 2 to get radius (approximate for off-center stim)... 5 in Gao
stimpars.stimdur = 4; %% duration of presentation of each unique stimulus (2s in Gao)
stimpars.isi = 4; %% time in seconds between end of one stimulus and beginning of next stimulus
...
setupvars.eye2screen_top_bottom = 14; %% distance in inches from eye to both top and bottom of stimulus screen
