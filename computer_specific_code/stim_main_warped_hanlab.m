function [] = stim_main_warped_hanlab()
...
%%%%% last updated 18/10/9 on thermaltake
...
% prerender_file = 'stimset_prerender_test_sizetuning1.mat'; % name of file where prerendered warped gratings are stored
prerender_file = 'stimset_prerender.mat';
save_stim_file = 1;
...
stimpars.diam_minmax = [60 60]; %% range of annulus diameters in degrees ([1 64] in Gao fig 1)
stimpars.stim_center = [960 540]; %%% waning: this value might not function if prerend.prerenderedLocAndDiam == true
stimpars.n_diams = 1;   % number of values of annulus diameter to 
stimpars.repetitions = 1; %% 10 in Gao
stimpars.isi = 4; %% inter-stimulus interval in seconds (onset to offset; 0.5 in Vaiceliunaite et al. 2013)
