...
%DRAW_GRATING Draw two concentric black/white drifting sine gratings of 
% different parameters separated by a ring.
%%% last updated 5/10/15

dur = 2; %% seconds
rect_center = [800 400];
% rect_center = folScreen('CloseAll')
...
Axes = [50 50; 70 70]; %% diameter of [hrz vert] ellipse axes in degrees; (approximate for off-center stim)... 2 each in Gao
Angle = [-30; 20];
sfreq = [0.01; 0.02]; % cycles per degree; Gao range ~0.0 then up1:1.6, peak ~0.06 cyc/deg
tfreq = [.8; 0.1]; % hz
