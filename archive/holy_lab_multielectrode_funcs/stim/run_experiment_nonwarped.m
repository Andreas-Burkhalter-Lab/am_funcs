function [] = run_experiment_nonwarped(rf_center)
...
%% add an option (arg 2 maybe) to input values for manual rf center  to redo rf mapping
...
%%%%% last edited 10/29/15 on stim comp
global screen_height eye2screen_top_bottom
...
savefile = strcat('ssstimdata',date); %% maybe customize more for each session/subject
...
stimpars.diam_inner = 0; %% diameter of inner grating in degrees... AB says it's OK to assume an average value for all cells
...
stimpars.sf_minmax = [0.2 0.2];       %% range of annulus spatial frequencies in cycles per degree (~[0.01 1.6] in Gao)
stimpars.tf_minmax = [1.5 1.5];       %% range of annulus temporal frequencies in hz (~[0.1 13] in Gao fig 1)
stimpars.diam_minmax = [46 50]; %% range of annulus diameters in degrees ([1 64] in Gao fig 1)
stimpars.angle_range = [0 360]; % range of linearly-space angles to present, not including the upper limit
stimpars.n_sfs = 1;      % number of values of annulus spatial frequency to present
stimpars.n_tfs = 0;      % number of values of annulus temporal frequency to present
stimpars.n_diams = 6;   % number of values of annulus diameter to present 
stimpars.nAngles = 1; %% number of times to present the stim set, using one of this number of evenly-spaced orientations 

stimpars.repetitions = 8; %% 10 in Gao
stimpars.stimdur = 2; %% duration of each stimulus presentation in seconds
stimpars.isi = 0.5; %% inter-stimulus interval in seconds (onset to offset; 0.5 in Vaiceliunaite et al. 2013)
...
surround_suppression_stim_nonwarped(stimpars, rf_center, pulsepars, savefile); 
