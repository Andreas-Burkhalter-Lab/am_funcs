function [out1,out2,out3,haxout] = ginput_with_axis(arg1)
%GINPUT_WITH_AXIS: like ginput, but also returns the handle(s) of clicked axes
% [X,Y,BUTTON,HAX] = ginput_with_axis(n)
% See the help for ginput.

% Modified from ginput by Timothy E. Holy, 2009.

out1 = []; out2 = []; out3 = []; y = []; haxout = [];
c = computer;
if ~strcmp(c(1:2),'PC')
...
if ~strcmp(tp,'none') && ~strcmp(tp,'x') && ~strcmp(tp,'micro'),
  if nargout == 1,
    if nargin == 1,
      out1 = trmginp%   [X,Y] = GINPUT(N) gets N points from the current axes and returns 
