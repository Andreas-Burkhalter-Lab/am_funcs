...
class balanced_neighborhood {
  ...
  public:
    ...
    int n_exceed;        // the # of neighbors required to exceed the balancing criterion by the given factor (pre-backtracking)
...
template <class dataType,class intType>
    class l2_operations {
      private:
        ...
        struct less_index {
          const intType *keyvec;
          less_index(const intType *keyin) {keyvec = keyin;}
          bool operator()(const int i1,const int i2) const {
            return keyvec[i1] < keyvec[i2];
          }
        };
        struct greater {
               ...
               msams_temporary_storage<dataType> &tmp)
{
  ...
  n = nbr_out.n_exceed = nbr_out.n_valid = 0;
  ...
  if (msams_criterion_met && ops.backtrack) {
    for (n = nbr_out.n_exceed; n > nbr_out.n_exceed - n_points_to_remove; n--) {
                         ...
                         const optionStruct &ops)
{
  ...
  while (!isdone) {
    ...
    if (using_base_neighbor_sorting) {
      ...
      nbrP = &nbr;
      n_extra += n_examined - nbr.n_exceed;
      if (niterp)
        niterp[1]++;
    } else {
