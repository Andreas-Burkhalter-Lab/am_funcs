function [dx,im2shift,err] = register_translation_multires(im1,im2,options)
...
% Copyright 2007 by Timothy E. Holy
  ...
  options = register_multigrid_options(im1,options);
  options = default(options,'n_vcycles',2);
  options = default(options,'dx_start',zeros(1,options.n_dims));
  
  dx = options.dx_start;
  err = imcompare(im1,{im2});
  for iter = 1:options.n_vcycles
    [dx,im2shift,err(iter+1)] = register_translation_vcycle(dx,im2, ...
						  options);
