function [g,psig,mu,old_val] = register_nonrigid(psi1,psi2,g,lambda,mu,options)
% REGISTER_NONRIGID: non-rigid multidimensional image registration
...
%
% Syntax:
%   [g,psig,mu,err] = register_nonrigid(psi1,psi2,g0,lambda,mu)
%   [g,psig,mu,err] = register_nonrigid(psi1,psi2,g0,lambda,mu,options)
% where
...
%     end
  ...
  while still_descending
    ...
    %
    % Add in the regularization penalty to the gradient
    %
    if (lambda > 0)
      for dimIndex = 1:n_dims
        grad_g(dimIndex,:) = grad_g(dimIndex,:) + ...
          (lambda*sqr_scalar) * (Hreg*g{dimIndex}(:))';
      end
    end
    is_lower = false;
    while (~is_lower && iter < options.iter_max && still_descending)
      ...
      %
      delta_g = nan(size(grad_g),'single');
      if (lambda == 0)
        delta_g = (-mu/Hscalar) * grad_g;
      else
        for dimIndex = 1:n_dims
          delta_g(dimIndex,:) = -grad_g(dimIndex,:)/((lambda*sqr_scalar)* Hreg + (Hscalar/mu)*I);
        end
      end
      for dimIndex = 1:n_dims
