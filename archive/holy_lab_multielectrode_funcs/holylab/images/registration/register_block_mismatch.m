function mismatch = register_block_mismatch(usz,fixed,moving,options)
...
%
% Syntax:
%   mismatch = register_block_mismatch(usz,fixed,moving,options)
% where
...
% Fixme: implement RGB/multi-valued images
  ...
  for blockIndex = 1:n_blocks
    ...
    if options.plot
      subplot(1,2,1); imshowrgb(fixedsnip,moving(x_src{:})); title(sprintf('block %d',blockIndex));
    end
    ...
    fftsz = 2*n + 2*maxshift;
    fftsz = ceil(fftsz/32) * 32; % round up on a 32 base, for speeding up of FFTN
...
%       fprintf('fixedpad, dim = %d: span_src = [%d %d], span_dest = [%d %d]\n',dimIndex,span_from_fixed(dimIndex,:),thisspan);
...
%       fprintf('moving_nan, dim = %d: span_nan = [%d %d]\n',dimIndex,thisspan);
      x_dest{dimIndex} = thisspan(1):thisspan(2);
    end
...
%       fprintf('movingpad, dim = %d: span_src = [%d %d], span_dest = [%d %d]\n',dimIndex,span_from_moving(dimIndex,:),thisspan);
    end
    movingpad(x_dest{:}) = moving(x_src{:});
    %% Compute the correlation    
    [numerator,denominator] = register_mismatch_nancorr(fixedpad,movingpad,wpad);    
    ...
    numerator(denominator < 1e-5) = 0;  % little pixel overlap, so we worry about roundoff errors
%   denominator(denominator == 0) = 1;  % prevent NaNs?
    ...
    if options.plot
      ...
      subplot(1,2,2); imagesc(mismatch{blockIndex}); axis image; title(sprintf('%d ',coord)); pause
    end
  end
