...
function UpdateFFMpegImage(ud)
im = read(ud.Movie,ud.currframe);
set(ud.himage,'cdata', im);
...
switch ud.MovieFormat
  ...
  case 'ffmpeg'
      ud.UpdateImageFcn = @UpdateFFMpegImage;
    otherwise
        error('Unrecognized movie format.');
end
set(hfig,'userdata',ud);
...
function jumptoframe(hfig,frameNumber)
  ud = get(hfig,'UserData');
  ud.currframe = frameNumber;
  ud.nextframe = frameNumber;
  ud.jumpto_frame = frameNumber;
