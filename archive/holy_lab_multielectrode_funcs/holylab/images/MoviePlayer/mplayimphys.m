function mov = mplayimphys(varargin)
%MPLAYIMPHYS Play an IMPHYS movie interactively.
%   MPLAYIMPHYS(A) opens a new movie player GUI and loads movie data
%   A into the player.  Multiple players may be used at one time.
%   A is an IMPHYS structure array.
%
%   MPLAYIMPHYS(A,options) supplies an options structure with the following
%   possible fields:
%     fps:  frames per second
%     busymode: 'drop' (skip frames if necessary to keep up with fps) or
%               'queue' (play every frame, even if slower than fps)
%     frameindex:  play only a subset of the frames in A, a vector of
%               frame indices that should be played.
%     clim:  color limits, for scaling the intensities
%     showstimulus: if true, displays a moving stair plot of the stimulus
%       as the movie advances.
%
% See also: MPLAY.
  
% Based on the nice MPLAY player written by D. Orofino.
% Modifications copyright 2005 by Timothy E. Holy
...
%                   'I'    MxNxF
%                   'RGB'  MxNx3xF
%                   'Sc'   Scaled (supply color limits, 'clim' option)
%
...
A = varargin{1+arg_offset};
if (nargin < 2+arg_offset)
  options = struct;
else
  options = varargin{2+arg_offset};
end
options.inputname = inputname(1);
options = mpip_options(options);

% Create and load GUI with movie
hfig = CreateGUI(options);
LoadMovie(hfig, A, options);% fmt, inputname(1), fps);
...
% --------------------------------------------------------
function hfig = CreateGUI(options)
...
hstimaxis = [];
if options.showstimulus
  hstimfigure = figure(...
    'position',[50 500 400 60],...
    'menubar','none',...
    'NumberTitle','off',...
    'Name','Stimulus',...
    'BackingStore','off',...
...
% --------------------------------------------------------
function UpdateGUIControls(hfig,isRunning)
% Update GUI state, including:
%   - button enable states/icons
%   - status bar text

if (nargin > 1)
  UpdateButtonEnables(hfig,isRunning)
else
...
% --------------------------------------------------------
function UpdateButtonEnables(hfig,isRunning)
% Button states:
%   Enabled=1
%   Disabled=0
...
if strcmp(get(ud.htimer,'Running'),'on'),
  ...
  if ud.paused,
    ...
    % Start from 1st frame when stopped
    ud.nextframe = 1;
...
% --------------------------------------------------------
function LoadMovie(hfig, A, options)

% Store basic movie info:
%
ud = get(hfig,'userdata');
ud.MovieFormat = options.fmt;
ud.Movie     = A;
ud.fps       = options.fps;
ud.varName   = options.title;
[r,c,f]      = MovieSizeInfo(A,options.fmt);
ud.frameRows = r;
ud.frameCols = c;
ud.numFrames = f;
if isfield(options,'frameindex')
  if (ischar(options.frameindex) && strcmp(options.frameindex,':'))
    ud.selframes = 1:length(A);
  else
    ud.selframes = options.frameindex;
  end
  ud.useSelframes = 1;
  ud.numFrames = length(ud.selframes);
else
  ud.useSelframes = 0;
end
ud.currframe = 1;
...
if isfield(options,'clim')
  if(ishandle(options.clim) && strcmp(get(options.clim, 'type'), 'image'))
     options.clim=get(get(options.clim, 'parent'), 'clim'); 
  end
  if (diff(options.clim) > 100 & isunix)
    % The range of values is large enough (compared to 1) that we can
    % effectively round to integers. In this case, it's faster (by almost
    % 2x) to use a direct colormap
    ud.cmap = graymap(options.clim);
    ud.cmapexpr = ['graymap(' num2str(options.clim(1)) ' ' ...
      num2str(options.clim(2)) ')'];
    set(hfig,'colormap',ud.cmap);
  else
    % The dynamic range may be compressed so that rounding to integers
    % destroys the information in the movie. In this case, we have to use a
    % scaled colormap
    set(ud.haxis,'CLim',options.clim);
    set(ud.himage,'CDataMapping','scaled');
  end
end

% Plot stimulus, if desired
if ishandle(ud.hstimaxis)
  stairs(ud.hstimaxis,[A.stimulus],'EraseMode','xor');
  x = [1 1];
  if ud.useSelframes
    x = ud.selframes(x);
  end
  hcposline = line(x,[0 max([A.stimulus])],'LineStyle','--',...
    'EraseMode','xor','Color','k');
  ud.stimcposline = hcposline;
end

% Store data, set name in figure title
set(hfig, ...
  'userdata', ud, ...
  'name', sprintf('Movie: %s', ud.varName));
...
SetupTimer(hfig,options);             % Updates FPS status readout via SetFPS
...
function UpdateImphysImage(ud)
  cframe = ud.currframe;
  if ud.useSelframes
    cframe = ud.selframes(cframe);
  end
  tmp = ud.Movie(cframe);
  if isa(tmp.image,'vimage')
    set(ud.himage,'cdata', eval(tmp.image));
  else
    set(ud.himage,'cdata', tmp.image);
  end
  if ishandle(ud.hstimaxis)
    x = ud.currframe;
    if ud.useSelframes
    ...
    set(ud.hstimaxis,'XLim',[-40 40]+x);
    set(ud.stimcposline,'XData',[x x]);
  end
  %fprintf('%d ',cframe);
  drawnow
...
%     case 'Sc'
        ud.UpdateImageFcn = @UpdateImphysImage;
%     otherwise
%         error('Unrecognized movie format.');
%end
set(hfig,'userdata',ud);
...
% --------------------------------------------------------
function SetupTimer(hfig,options)
...
udtimer.hfig = hfig;
udtimer.dropframes = 1;
if strcmp(options.busymode,'queue')
  udtime.dropframes = 0;
end
udtimer.lasttime = [];
...
thistime = datenum(user.Data.time);
dframe = 1;  % The number of frames to advance
if ud.dropframes
  if ~isempty(ud.lasttime)
    dt = 86400*(thistime-ud.lasttime);  % Time since last call in seconds
    dframe = ceil(dt/get(hco,'Period'));
  end
  ud.lasttime = thistime;
  set(hco,'userdata',ud);
end
...
if (ud.currframe == ud.numFrames),
    ...
    if ud.fbmode % && ~ud.fbrev,  % unnecessary qualifier --- always will be false here
        ...
        ud.nextframe = max(ud.currframe-dframe,1);
        shouldStop = 0;
...
elseif (ud.currframe == 1),
    ...
    if ud.fbmode && ud.fbrev,
        ...
        ud.nextframe = min(ud.currframe+dframe,ud.numFrames);
        shouldStop = 1;
    else
        ud.nextframe = min(1+dframe,ud.numFrames);  % ud.currframe+1
        shouldStop = 0;
    end
    
else
    if ud.fbmode && ud.fbrev,
        ud.nextframe = max(ud.nextframe - dframe,1);% next frame, reverse play
    else
        ud.nextframe = min(ud.nextframe + dframe,ud.numFrames);  % next frame, fwd play
    end
    shouldStop = 0;
end
set(hfig,'userdata',ud);
...
% ----------------------------------------
function options = mpip_options(options)
  if ~isfield(options,'title')
    %options.title = options.inputname;
    options.title = '';
  end
  if ~isfield(options,'fps')
    options.fps = 30;
  end
  if ~isfield(options,'busymode')
    options.busymode = 'queue';
  end
  if ~isfield(options,'frameindex')
    options.frameindex = ':';
  end
  if ~isfield(options,'fmt')
    options.fmt = 'Sc';  % Scaled
  end
  if ~isfield(options,'showstimulus')
    options.showstimulus = 0;
  end
