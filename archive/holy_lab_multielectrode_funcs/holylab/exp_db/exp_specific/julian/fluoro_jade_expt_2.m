...
median_img1 = double(median(median(filt_img1(:),2),1));
max_img1 = double(max(max(filt_img1(:),[],2),[],1));
filt_img1([1700:1720], [300:300+100/0.71]) = 65535; % scalebar 100 um
control_img_axis = axes('parent', mainfig, 'units', 'normalized', 'position', [.01 .51 .45 .45]);
control_img_handle = imshow(filt_img1);%, 'parent', control_img_axis);
set(control_img_axis, 'clim', [0.2*median_img1 0.9*max_img1], 'xdir', 'reverse', 'visible', 'off');
...
% filt_img2([1700:1720], [400:400+100/0.71]) = 65535; % scalebar 100 um
...
%% Set up 240 min chamber+suction image [fjc_expts{25}.data_locations{3} =
% {'/mnt/julian_004/julian/FluoroJade_C_expts/2008_04_07/sampled/image_processing/2008_04_07_sampled_sec7_20x_max_merge_filt.tif';}
hold on;
img2 = imread(fjc_expts{25}.data_locations{3});
filt_img2 = imfilter(img2, punctate_filter_jpm_1);
median_img2 = double(median(median(filt_img2(:),2),1));
max_img2 = double(max(max(filt_img2(:),[],2),[],1));
filt_img2([1700:1720], [400:400+100/0.71]) = 65535; % scalebar 100 um
chamber_img_axis = axes('parent', mainfig, 'units', 'normalized', 'position', [.28 .51 .45 .45]);
chamber_img_handle = imshow(filt_img2);% , 'parent', chamber_img_axis);
set(chamber_img_axis, 'clim', [0.2*median_img2 0.7*max_img2], 'xdir', 'reverse');
chamber_img_title = title(chamber_img_handle, 'string', '4 hr. Chamber + microsuction',...
                                              'fontsize', 12, 'fontweight', 'bold');
% --------end chamber+suction image setup-----------------------------------------

%% Set up 240 min bench image [fjc_expts{22}.data_locations{1} = 
% '/mnt/julian_004/julian/FluoroJade_C_expts/2007_10_06/sample_7/image_processing/2007_10_06_sample7_section6_max_merge.tif';
hold on;
img3 = imread(fjc_expts{22}.data_locations{1});
...
filt_img3([1750:1770], [300:300+100/0.71]) = 65535; % scalebar 100 um
...
set(bench_img_axis, 'clim', [0.2*median_img3 0.9*max_img3]);
bench_img_title = title(bench_img_handle, 'string', '4 hr. Warmed Bench',...
                                              'fontsize', 12, 'fontweight', 'bold');
