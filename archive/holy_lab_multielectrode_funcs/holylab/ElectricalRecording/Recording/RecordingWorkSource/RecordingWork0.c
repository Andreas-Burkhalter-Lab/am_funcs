...
void RecordingWork(int16 nchan,int16 *channels,float64 scanrate,uint32 nscans,uint32 buffersize,int trigger,int gain,
...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
	...
	RecordingWork(nchan,channels,scanrate,nscans,buffersize,trigger,gain,npix,axHP,textHP,usrheader,filename);
				...
				int npix,const mxArray *axHP,const mxArray *textHP,const char *usrheader,const char *filename)
{
	...
	mexPrintf("secsPerHalfBuff = %g,buffersize %d,scanrate %g\n",secsPerHalfBuff,buffersize,scanrate);
	...
	if (strlen(filename) > 0) {
		...
		dblD = mxGetPr(lhs[0]);
		sprintf(ctemp,"%d:%d:%f",round(dblD[3]),round(dblD[4]),dblD[5]);
		mxDestroyArray(lhs[0]);
	...
	patchHP = mxCreateDoubleMatrix(1,64,mxREAL);
	...
	*scalarMxD = 0;
	...
	status = DAQ_Rate(scanrate*nchan, units, &sampleTimebase, &sampleInterval );
	errcheck( status, "DAQ_Rate" );
    ...
    // Start scanning!
    status = SCAN_Start(device,buffer,count,sampleTimebase,sampleInterval,sampleTimebase,0);
	...
	while(( loopCount < numberOfHalfBuffersToRead ) && ( status == noError ) && gIsRecording) {
		...
		if(( isHalfReady == 1 ) && ( status == noError )) {
			...
			rhs[0] = patchHP;
			mexCallMATLAB(0,lhs,1,rhs,"delete");
...
/*
				mexCallMATLAB(1,lhs,3,rhs,"fill");
				patchHD[channels[i]] = mxGetScalar(lhs[0]);
				mxDestroyArray(lhs[0]);	// ??
*/
			}
			// Inform user of # of secs remaining
			sprintf(secsLeft,"%d seconds remaining",(int)round(secsPerHalfBuff*(numberOfHalfBuffersToRead-loopCount)));
			rhs[0] = (mxArray*) textHP;
			rhs[1] = stringStrMxP;
			rhs[2] = mxCreateString(secsLeft);
