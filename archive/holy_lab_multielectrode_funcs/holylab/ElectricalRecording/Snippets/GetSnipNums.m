function [chans,nsnips,sniprange] = GetSnipNums(filenames,options)
...
%
% [chans,nsnips,sniprange] = GetSnipNums(filenames,options)
% lets you supply the following options:
%   machfmt: control the endian status of the snippet files (legacy files)
  ...
  for i = 1:length(filenames)
    if (nargin < 2)
      h = readheader(filenames{i});
    else
      h = readheader(filenames{i},options);
    end
