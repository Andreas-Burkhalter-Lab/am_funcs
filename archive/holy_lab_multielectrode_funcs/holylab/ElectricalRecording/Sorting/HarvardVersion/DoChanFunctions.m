function DoChanFunctions(action,h)
   ...
   switch(action)
	 ...
	 spikes = cell(nsel,1);
	 for i = 1:nsel
	    spikes{i} = LoadIndexSnippetsMF(sfiles,channel,wvindx(i,:),params);
	 end
	 ...
	 [tnew,indxnew] = GroupChannel(spikefiles,channel,...
				       filters,subrange,blocksize,replclust+clustnumoffset-1,wvindx,params);
	 ...
	 for i = 1:min(nclust,7)
	    ...
	    snips = LoadIndexSnippetsMF(spikefiles,channel,viewindx,params);
	    ...
	    npix = pos(2);
	    nbins = ceil(npix/2);
	    nPerBin = autocorrspikecat(tsecs,actime,nbins);
	    binwidth = actime/nbins;
	    xc = linspace(binwidth/2,actime-binwidth/2,nbins);
	    co=get(haxc(2,i),'colorOrder');
	    hbar = bar(xc,nPerBin,1,'k');
	    set(hbar,'FaceColor',co(i,:),'EdgeColor',co(i,:));
	    axis tight
	    ylim = get(gca,'YLim');
	    if (ylim(1) < 0)
	 ...
	 [snip,tsnip,h] = LoadSnip(spikefiles{flindx},channel,params);
