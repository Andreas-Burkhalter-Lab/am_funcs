...
%   max_snips_to_cluster (default 1e6): the name says it all...almost,
...
% Copyright 2005-2006 by Timothy E. Holy
  ...
  options = default(options,'normalize',true);
  options = default(options,'n_landmarks',500);
  options = default(options,'n_closest_landmarks',5);
  options = default(options,'n_dimensions_per_channel',4);
  options = default(options,'n_landmarks_channel',200);
  ...
  if ~isfield(options,'max_snips_to_cluster')
    options.max_snips_to_cluster = 1e5;
  end
