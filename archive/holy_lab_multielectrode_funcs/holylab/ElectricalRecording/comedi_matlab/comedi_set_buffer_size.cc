    ...
    int nrhs, const mxArray *prhs[])
{
  ...
  if (nrhs != 3)
    mexErrMsgTxt("syntax: comedi_set_buffer_size(devicename,subdevice,buffersize)");
  ...
  if (!mxIsNumeric(curarg) || mxIsComplex(curarg) || !IsScalar(curarg))
    mexErrMsgTxt("comedi_set_buffer_size: subdevice must be a real scalar (nonneg integer)");
  subdevice = (unsigned int) mxGetScalar(curarg);
  /* Buffer size */
  curarg = prhs[2];
  if (!mxIsNumeric(curarg) || mxIsComplex(curarg) || !IsScalar(curarg))
    mexErrMsgTxt("comedi_set_buffer_size: buffer size must be a real scalar (nonneg integer)");
  bufsz = (unsigned int) mxGetScalar(curarg);

  comedi_set_buffer_size(dev,subdevice,bufsz);
} 
