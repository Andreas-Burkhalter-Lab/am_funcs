    ...
    int nrhs, const mxArray *prhs[])
{
  ...
  if (nrhs < 2)
    mexErrMsgTxt("comedi_get_n_channels: requires 2 input arguments");
  ...
  if (!mxIsNumeric(curarg) || mxIsComplex(curarg) || !IsScalar(curarg))
    mexErrMsgTxt("comedi_get_n_channels: subdevice must be a real scalar (nonneg integer)");
  subdevice = (unsigned int) mxGetScalar(curarg);

  ret = comedi_get_n_channels(dev,subdevice);
  if (ret < 0) {
    comedi_perror("comedi_get_n_channels");
    mexErrMsgTxt("comedi_get_n_channels: error getting number of channels from device");
  }
