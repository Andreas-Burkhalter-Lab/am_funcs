...
%            .resampled_raw:     }
%            .maxsum_valvegroups:} 
%            .maxsum_valveids    }} if not present will be set by defaults of 'meea_maxsum_valvegroups'
%            .maxsum.(data_type).(analysis_method).(data_processing)
%                   >> 'data_type' refers to various metrics (mean, sem, ttest, ranksum etc.)
%                   >> 'analysis_method' refers to the various methods of condensing a response into a scalar value
%                       (examples: dr, drwindow, drcumulative)
%                   >> 'data_processing) refers to the form of the data
%                       (currently 'raw' or 'filtered' if passed through a filter)
%            .version.maxsum: current analysis version
%
% See also MULTI_ELECTRODE_EPHYS_ANALYZE MEEA_MAXSUM_TSTART MEEA_MAXSUM_VALVEGROUP MEEA_MAXSUM_TIMEWEIGHT
