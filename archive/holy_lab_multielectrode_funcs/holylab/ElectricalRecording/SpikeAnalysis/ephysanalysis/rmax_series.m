function [rtrial,tpeak] = rmax_series(ephys,tags,trange,fieldname,nbins,tmin)
    ...
    for tagI = 1:ntags
      ...
      for rptI = 1:nrpts
        ...
        if ~isempty(tspike)
            ...
            flag = tspike >= trange(1) & tspike < trange(2);
            tspike_baseline{tagI}{rptI} = tspike(flag) - trange(1);
            % Extract spikes in the "response" interval
            flag = tspike >= trange(2) & tspike < trange(3);
            tspike = tspike(flag) - trange(2);   
