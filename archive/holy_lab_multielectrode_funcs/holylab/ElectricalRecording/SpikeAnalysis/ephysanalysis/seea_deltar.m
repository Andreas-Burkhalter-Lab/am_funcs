function analysis_out = seea_deltar(ephysin, analysis_in)
% seea_deltar calculates delta_r values over the default time range
% (up to stimulus presentation + 2 seconds following presentation)... in future
% can be expanded.  Will choose "celltimes" by default, 
% will add fields:
...
% to the input analysis structure.

% Written: (2008_05_01-2008_05_09 JPM)
        ...
        if ~isempty(ephysin(ephys_idx).(choice){cell_idx})
            ...
            if isfield(analysis_out, 'defined_subintervals')
                if ~iscell(analysis_out.defined_subintervals)
                    if analysis_out.defined_subintervals == 'all'
                        ...
                        e_vals = [];
                        for s_idx = 0:5
                            for e_idx =  1:round(diff(ephysin(ephys_idx).scanrange/ephysin(ephys_idx).scanrate)+ephysin(ephys_idx).toffset)
                                if isempty(s_vals)
                                    s_vals = s_idx;
                                else
                                    s_vals(end+1) = s_idx;
            ...
            for se_idx = 1:size(s_vals,2)
                if e_vals(se_idx) > s_vals(se_idx)
                    fldname = sprintf('s%de%d', s_vals(se_idx), e_vals(se_idx));
