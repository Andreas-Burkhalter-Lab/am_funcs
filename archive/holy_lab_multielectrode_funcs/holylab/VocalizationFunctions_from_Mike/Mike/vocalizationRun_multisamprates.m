...
% AM edit 9/5/14: In this version, different sample rates are specified for
% each subject. Begin with an n x 1 vector called "sratess" (n=number of 
% subjects), where each value is  the sampling rate for the corresponding 
% subject. Make sure the order is the same as that in which the subjects 
% are listed by 'dir' and in the output 'data.m.' The input sample rate to
% vocstartupVariables will be ignored. Note that frequency range will be
% the same for all subjects. 
% All changes from the original vocalizationRun are marked with comments. 
clearvars -except srates; close all hidden;                   %%%%% changed from vocalizationRun

%  Following lines added by AM 9/3/14: set 'threshold_override' to 1 if you 
...
% Set 'skipchecks' to 1 to skip the step of checking sonograms and determining metadata import. 
...
skipchecks = 1;
threshold_override = 0;         % set to 1 to prescribe the same threshold for all subjects, set to 0 to not override thresholds
thresh_override_value = 814.3500 * threshold_override;
threshold_multiply = 1;         % set to 1 to multiply the auto-generated thresholds, set to 0 to not multiply thresholds
thresh_multiply_value = 0.45 ^ threshold_multiply;

[rootPath,...
    nfft,...
    freqRange,...
    ~,...                       %%%%% changed from vocalizationRun
    whisoptions,...
    micFactor,...
    vpk,...
...
for i = 1:numel(wavlist)
    [threshold(i),noiseMean(i),noisesd(i)] = ...
        vocThreshold(wavlist(i).name,nfft,srates(i),threshLength,freqRange,thresholdType,1);     %%%%% changed from vocalizationRun
...
for i = 1:numel(wavlist)
    ...
    clipIndices{i} = sound2sng(wavlist(i).name,sngparms,[wavlist(i).name(1:end-4),'.SNG']);      %% removed '.wav' from .sng filename - AM 9/12/14
...
cd([rootPath,'\Outputs']);
sniplist = dir('snips*');
for sub = 1:length(srates)                      %%%%% changed from vocalizationRun
    f{sub} = linspace(0,srates(sub)/2,nfft/2 + 1);      %%%%% changed from vocalizationRun
end                                             %%%%% changed from vocalizationRun
...
for i = 1:numel(sniplist)
    ...
    [syllobjects(j).syllObj,...
     ...
     syllobjects(j).syllPfadj] = syllableObjects(snips{j},f{i},threshold(i));
...
for i = 1:numel(data)
    ...
    if data(i).nwhis > 0
        ...
        for j = 1:numel(snips)
            ...
            x = (1/(srates(i)*nfft)).*(x.^2);  % square and scale to V*V/Hz Units                        %%%%% changed from vocalizationRun
