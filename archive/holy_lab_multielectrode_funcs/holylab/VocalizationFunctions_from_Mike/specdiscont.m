...
pow = sum(sng)';            % total power in each time bin
[i,j,s] = find(sng); %[f,t,value]
% The "normalized" sonogram
% (sum of each column is 0 or 1)
nsng = sparse(i,j,s./pow(j),size(sng,1),size(sng,2)); % value divided by total power in time bin 
