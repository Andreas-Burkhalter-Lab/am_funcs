function sound2sng(infilename,sngparms,outfilename)
...
%  CutClippingOut:  if 1 then times of clipping get edited out.
...
%        sngparms.nfreq = 256;
%        sngparms.freqrange = [25000 125000];
...
if ~isfield(sngparms,'CutClippingOut')
  sngparms.CutClippingOut = 0;
end
%% Read the header
if iswav
...
for i = 1:nblocks
  ...
  if any(microphoneVoltage <= header.voltageMin) || any(microphoneVoltage >= header.voltageMax)
    ...
    if sngparms.CutClippingOut
      warning('cutting clipping out');
      microphoneVoltage(find(microphoneVoltage <= header.voltageMin))=[];
      microphoneVoltage(find(microphoneVoltage >= header.voltageMax))=[];
    end
  end
