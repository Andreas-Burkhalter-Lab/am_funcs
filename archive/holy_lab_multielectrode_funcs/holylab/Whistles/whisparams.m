...
% The main tool is to replace the sonogram by its dominant frequency at
...
% Copyright 2004 Timothy E. Holy.
  ...
  p.numjumps = cell(1,nwhis);
  p.meanfluctuationsnojumps = cell(1,nwhis);
  ...
  for i = 1:nwhis
    ...
    if (length(freqvst) > 1)
      ...
      if (length(freqvst) >= 3*length(b))
        ...
        p.numjumps5000{i} = sum(diff(freqvst)>5000);
         p.numjumps3000{i} = sum(diff(freqvst)>3000);
          p.numjumps1000{i} = sum(diff(freqvst)>1000);
           p.numjumps500{i} = sum(diff(freqvst)>500);
            p.biggest_abs_jump{i} = max(abs(diff(freqvst)));
             p.numjumps_abs5000{i} = sum(abs(diff(freqvst))>5000);
         p.numjumps_abs3000{i} = sum(abs(diff(freqvst))>3000);
          p.numjumps_abs1000{i} = sum(abs(diff(freqvst))>1000);
           p.numjumps_abs500{i} = sum(abs(diff(freqvst))>500);
        p.meanfluctuationsnojumps{i}= mean(p.dadjf{i}(find(diff(freqvst)<5000)));
      end
    end
