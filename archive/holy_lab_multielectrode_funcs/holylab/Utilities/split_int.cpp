...
//syntax intarray=split_int(astring, delimiter)
//e.g. a=split_int('1 2 3', ' '); 
//    we got a=[1 2 3]
...
mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
    ...
    vector<long> intList;
    for(int i=0; i<stringList.size(); ++i)intList.push_back(atol(stringList[i].c_str()));

    //we can always assume one LHS arg there---if user does not supply one explicitly, it is variable ans:
    plhs[0] = mxCreateDoubleMatrix(1, intList.size(), mxREAL);
    double* p=mxGetPr(plhs[0]);
    copy(intList.begin(), intList.end(), p); //todo: cast long to double, may lose precision
