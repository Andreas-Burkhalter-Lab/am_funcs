function handle=nonblock_inputdlg(Prompt, Title, NumLines, DefAns, moreOptions)
...
%
%  NumLines: specifies the number of lines
...
%
%  DefAns: specifies the default
%     answer to display for each Prompt.  DefAns must contain the same
%     number of elements as Prompt and must be a cell array.
%
%  moreOptions(optional): it must include 3 fields:
%     Resize='off': on' or 'off';
%     resultIndex=2: where to put user's inputs (the index to root's userdata property)
%     Interpreter='none': 'tex' or 'none'.  The interpreter applies to the prompt strings.
%
%post:
%  handle: the handle of the input dialog
%  ttRootUserdata=get(0, 'userdata'); 
%  ttRootUserdata{resultIndex} is the cell array for user's input strings
%                                 (empty cell if user clicked Cancel)
%
%  Examples:
%
...
%  lineNo=1;
%  AddOpts.Resize='on';
%  AddOpts.resultIndex=3;
%  AddOpts.Interpreter='tex';
%  answer=inputdlg(prompt,dlgTitle,lineNo,def,AddOpts);
%
%  See also inputdlg, TEXTWRAP, QUESTDLG, WAITFOR.

%  Loren Dean   May 24, 1995.
%  Copyright 1998-2002 The MathWorks, Inc.
%  $Revision$

%  modified by jason 6/27/2003, based on inputdlg.m
...
if nargin==5 & isstruct(moreOptions),
  Interpreter=moreOptions.Interpreter;
  resultIndex=moreOptions.resultIndex;
  Resize=moreOptions.Resize;
else
   Interpreter='none';
   resultIndex=2;
   Resize='off';
...
handle=InputFig; % set return value
...
%CBString='set(gcbf,''UserData'',''Cancel''); close;';
CBString=['ttRootUserData=get(0, ''userdata''); ttRootUserData{'  num2str(resultIndex) '}={}; set(0, ''userdata'', ttRootUserData); close;'];
        % if user clicked cancel, clear result to be empty cell array
...
% disp('here');
% CBString=['edits=findobj(gcf, ''style'', ''edit''); inputs=get(edits, ''string''); ttRootUserData=get(0, ''userdata''); ttRootUserData{'  num2str(resultIndex) '}=inputs; set(0, ''userdata'', ttRootUserData); close;'];
CBString=['ttRootUserData=get(0, ''userdata''); edits=ttRootUserData{' num2str(resultIndex) '}; inputs=get(edits, ''string''); ttRootUserData{'  num2str(resultIndex) '}=inputs; set(0, ''userdata'', ttRootUserData); close;'];
        % what findobj returns is not in the order of creating these
        % objects
...
ttRootUserData=get(0, 'userdata');
ttRootUserData{resultIndex}=EditHandle; % to avoid to use findobj
set(0, 'userdata', ttRootUserData);
