function varargout=plot2svg(param1,id,pixelfiletype,options)
...
options = default(options,'renderaxes',true);
...
for j=length(ax):-1:1
    ...
    if strcmp(currenttype,'axes')
        group=group+1;
        groups=[groups group];
        group=axes2svg(fid,id,ax(j),group,paperpos,options);
        axfound=1;
...
function group=axes2svg(fid,id,ax,group,paperpos,options)
global fixcolorptr
global colorname
global PLOT2SVG_globals
...
if strcmp(get(ax,'Visible'),'on')
    ...
    if (ticklength(1) ~= 0)
        if axlimx(1)~=axlimx(2)
            ...
            xg_label_end = interp1([axlimx(1) axlimx(2)],[x_label_end1 x_label_end2],axxtick);
            yg_label_end = interp1([axlimx(1) axlimx(2)],[y_label_end1 y_label_end2],axxtick);            
            for k = 1:length(x_axis_point_index)
                ...
                xg_line_start = interp1([axlimx(1) axlimx(2)],[x(x_axis_point_index(k)) x(edge_neighbours(x_axis_point_index(k),1))],axxtick);
                yg_line_start = interp1([axlimx(1) axlimx(2)],[y(x_axis_point_index(k)) y(edge_neighbours(x_axis_point_index(k),1))],axxtick);
                xg_line_end = interp1([axlimx(1) axlimx(2)],[x_tick_end1 x_tick_end2],axxtick);
                yg_line_end = interp1([axlimx(1) axlimx(2)],[y_tick_end1 y_tick_end2],axxtick);
                for i = valid_xsticks
    ...
    if (ticklength(1) ~= 0)
        if axlimy(1)~=axlimy(2)
            ...
            xg_label_end = interp1([axlimy(1) axlimy(2)],[x_label_end1 x_label_end2],axytick);
            yg_label_end = interp1([axlimy(1) axlimy(2)],[y_label_end1 y_label_end2],axytick);            
            for k = 1:length(y_axis_point_index)
                ...
                xg_line_start = interp1([axlimy(1) axlimy(2)],[x(y_axis_point_index(k)) x(edge_neighbours(y_axis_point_index(k),2))],axytick);
                yg_line_start = interp1([axlimy(1) axlimy(2)],[y(y_axis_point_index(k)) y(edge_neighbours(y_axis_point_index(k),2))],axytick);
                xg_line_end = interp1([axlimy(1) axlimy(2)],[x_tick_end1 x_tick_end2],axytick);
                yg_line_end = interp1([axlimy(1) axlimy(2)],[y_tick_end1 y_tick_end2],axytick);
                for i = valid_ysticks
                    line2svg(fid,grouplabel,axpos,[xg_line_start(i) xg_line_end(i)],[yg_line_start(i) yg_line_end(i)],scolorname,'-',linewidth)
                end
    ...
    if (ticklength(1) ~= 0)
        if axlimz(1)~=axlimz(2)
            ...
            xg_line_start = interp1([axlimz(1) axlimz(2)],[x(z_axis_point_index) x(edge_neighbours(z_axis_point_index,3))],axztick);
            yg_line_start = interp1([axlimz(1) axlimz(2)],[y(z_axis_point_index) y(edge_neighbours(z_axis_point_index,3))],axztick);
            xg_line_end = interp1([axlimz(1) axlimz(2)],[x_tick_end1 x_tick_end2],axztick);
            yg_line_end = interp1([axlimz(1) axlimz(2)],[y_tick_end1 y_tick_end2],axztick);
            xg_label_end = interp1([axlimz(1) axlimz(2)],[x_label_end1 x_label_end2],axztick);
            yg_label_end = interp1([axlimz(1) axlimz(2)],[y_label_end1 y_label_end2],axztick);            
            for i = valid_zsticks
                line2svg(fid,grouplabel,axpos,[xg_line_start(i) xg_line_end(i)],[yg_line_start(i) yg_line_end(i)],scolorname,'-',linewidth)
            end
...
for i=length(axchild):-1:1
    ...
    elseif strcmp(get(axchild(i),'Type'),'line')
        ...
        switch marker
            ...
            case 'square',group=group+1;,patch2svg(fid,group,axpos,x'*ones(1,5)+ones(length(linex),1)*[-1 -1 1 1 -1]*markersize,y'*ones(1,5)+ones(length(liney),1)*[-1 1 1 -1 -1]*markersize,markerfacecolorname,'-',linewidth,markeredgecolorname, 1, 1);
            case 'diamond',group=group+1;,patch2svg(fid,group,axpos,x'*ones(1,5)+ones(length(linex),1)*[-0.7071 0 0.7071 0 -0.7071]*markersize,y'*ones(1,5)+ones(length(liney),1)*[0 1 0 -1 0]*markersize,markerfacecolorname,'-',linewidth,markeredgecolorname, 1, 1);
            case 'pentagram',group=group+1;,patch2svg(fid,group,axpos,...
            ...
            case 'hexagram',group=group+1;,patch2svg(fid,group,axpos,...
                ...
                elseif strcmp(get(axchild(i),'FaceColor'),'none')
                    ...
                    elseif size(pointc,1)==size(faces,1)
                        if strcmp(get(axchild(i),'FaceColor'),'flat')
                            ...
                            facecolor = pointc(p,:);
                            cdata = pointc(p,:);     % TO DO: color interpolation
                            flat_shading = 0;
                        end
    ...
    elseif strcmp(get(axchild(i),'Type'),'image')
        ...
        [x,y,z] = project(pointx,pointy,zeros(size(pointx)),projection); 
        pointc=get(axchild(i),'CData');
        if strcmp(get(axchild(i),'CDataMapping'),'scaled')
            clim=get(ax,'CLim');
        ...
        if strcmp(get(ax,'DataAspectRatioMode'),'manual')
            pointsx=(min(x)*axpos(3)+axpos(1))*paperpos(3);
            pointsy=(1-(max(y)*axpos(4)+axpos(2)))*paperpos(4);
        else
