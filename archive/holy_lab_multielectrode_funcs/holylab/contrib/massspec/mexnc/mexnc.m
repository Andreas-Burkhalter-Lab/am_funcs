...
%    OPeNDAP
%    ------------------ 
%    You can control which version of the mex-file is invoked with a preference.
%    For example, to alternate usage between OPeNDAP-enabled and non-OPeNDAP, 
%    you would do the following:
%
%        setpref('MEXNC','FUNCSTR','vanilla_mexnc');
%        ... do normal mexnc stuff ...
%        setpref('MEXNC','FUNCSTR','opendap_mexnc');
%        ... do stuff that requires opendap ...
%
%    See the README for further details about OPeNDAP.
%       
...
%
% Preprocess netcdf-2 inputs
op = lower(varargin{1});
switch op
	case 'inquire'
		[varargout{:}] = handle_inquire(funcstr,nargout,varargin{:});
...
%----------------------------------------------------------------
function varargout = handle_inquire(funcstr,my_nargout,varargin)
% Funnel the INQUIRE operation thru INQ instead.

varargout = cell(1, my_nargout);
outs = cell(1, 5);

% Get all five outputs.
[outs{:}] = mexnc('INQ',varargin{2:end});
if my_nargout == 1
    % In this case, return all the outputs as a single vector.
	for j = 1:numel(outs)
		varargout{1}(j) = outs{j};
	end
else
	for j = 1:numel(outs)
		varargout{j} = outs{j};
