...
% Normalization bugfix by Timothy E. Holy, May 2009
...
for j = 1:length(n)
    ...
    if isnorm
      %normconst = sqrt((1+(m(j)~=0))*(n(j)+1)/pi);  % original value
      normconst = sqrt(2*(n(j)+1)/(1+(m(j)==0)));  % the correct value (TEH May2009)
      y(:,j) = y(:,j)*normconst;
    end
end
