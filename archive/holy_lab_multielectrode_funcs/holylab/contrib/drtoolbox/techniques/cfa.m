...
% Maastricht University, 2007
    ...
    min_var = 0;                     % minimum STD of Gaussians
    X = X';
    [D n] = size(X);
    c = no_analyzers;
    d = no_dims;
    ...
    for j=1:c
        tmp = zeros(D, D);
        tmp(1:size(tmp, 2) + 1:end) = rand(D, 1);
        Psi(:,:,j) = tmp;
    end
    ...
    while iter < no_iterations
        ...
        for j=1:c
            ...
            % Compute Vc and m
            Vc(:,:,j) = inv(SigmaC(:,:,j)) + tmpProd;
            m(:,j) = Kappa(:,j) + inv(Vc(:,:,j)) * Lambda(:,:,j)' * invPsi(:,:,j) * mean(Xnc, 2);
        end
        ...
        for i=1:n
            ...
            tmp(1:size(tmp, 2) + 1:end) = max(min_var, tmp(1:size(tmp, 2) + 1:end));
            SigmaN(:,:,i) = inv(tmp);           % code above gave us inv(SigmaN)
        end
