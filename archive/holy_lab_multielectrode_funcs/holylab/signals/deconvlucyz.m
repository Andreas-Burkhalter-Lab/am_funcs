function J = deconvlucyz(varargin)
%DECONVLUCYZ Deblur image using Lucy-Richardson method.
%   J = DECONVLUCYZ(I,PSF) deconvolves image I using Lucy-
...
%   point-spread function PSF and possibly by adding noise. This is a
%   trivial modification of Matlab's "deconvlucy" optimized for a specific
%   case, where the PSF has extent only along the z-axis. PSF must be a
%   1-by-1-by-n vector.
%
%   IMPORTANT NOTE: before running this function, you need to call
...
%   J = deconvlucyz(I,PSF,NUMIT)
%   J = deconvlucyz(I,PSF,NUMIT,DAMPAR)
%   J = deconvlucyz(I,PSF,NUMIT,DAMPAR,WEIGHT)
%   J = deconvlucyz(I,PSF,NUMIT,DAMPAR,WEIGHT,READOUT)
%   J = deconvlucyz(I,PSF,NUMIT,DAMPAR,WEIGHT,READOUT,SUBSMPL), where
%
...
%   ringing use I = EDGETAPER(I,PSF) prior to calling deconvlucyz.
%
%   Note also that deconvlucyz allows you to resume deconvolution starting
%   from the results of an earlier deconvlucyz run. To initiate this
...
%   array into the next deconvlucyz call. The input cell array can contain
%   one numeric array (on initial call), or four numeric arrays (when it
%   is the output from a previous run of deconvlucyz). The output J
...
%      J1 = deconvlucyz(BlurredNoisy,PSF);
%      J2 = deconvlucyz(BlurredNoisy,PSF,20,sqrt(V));
%      J3 = deconvlucyz(BlurredNoisy,PSF,20,sqrt(V),WT);
%      subplot(221);imshow(BlurredNoisy);
%                     title('A = Blurred and Noisy');
%      subplot(222);imshow(J1);
%                     title('deconvlucyz(A,PSF)');
%      subplot(223);imshow(J2);
%                     title('deconvlucyz(A,PSF,NI,DP)');
%      subplot(224);imshow(J3);
%                     title('deconvlucyz(A,PSF,NI,DP,WT)');
%
...
if (size(PSF,1) > 1 || size(PSF,2) > 1)
  error('For deconvlucyz, the PSF may only have extent along the 3rd dimension');
end
...
J{2} = J{2}(idx{:});
scale = real(ifft(bsxfun(@times,conj(H),fft(WEIGHT(idx{:}),[],3)),[],3)) + sqrt(eps);
clear WEIGHT;
DAMPAR22 = (DAMPAR.^2)/2;
...
for k = lambda + (1:NUMIT)
  ...
  CC = corelucyz(Y,H,DAMPAR22,wI,READOUT,SUBSMPL,idx,vec,num);

  % 3.c Determine next iteration image & apply positivity constraint
  J{3} = J{2};
  J{2} = max(Y.*real(ifft(bsxfun(@times,conj(H),CC),[],3))./scale,0);  
  clear CC;
  J{4} = [J{2}(:)-Y(:) J{4}(:,1)];
end
clear wI H scale Y;
...
%
PSF = varargin{2};%      deconvlucyz(I,PSF)
switch nargin
case 3,%                 deconvlucyz(I,PSF,NUMIT)
  NUMIT = varargin{3};
case 4,%                 deconvlucyz(I,PSF,NUMIT,DAMPAR)
  NUMIT = varargin{3};
  DAMPAR = varargin{4};
case 5,%                 deconvlucyz(I,PSF,NUMIT,DAMPAR,WEIGHT)
  NUMIT = varargin{3};
  DAMPAR = varargin{4};
  WEIGHT = varargin{5};
case 6,%                 deconvlucyz(I,PSF,NUMIT,DAMPAR,WEIGHT,READOUT)
  NUMIT = varargin{3};
  DAMPAR = varargin{4};
  WEIGHT = varargin{5};
  READOUT = varargin{6};
case 7,%                 deconvlucyz(I,PSF,NUMIT,DAMPAR,WEIGHT,READOUT,SUBSMPL)
  NUMIT = varargin{3};
  DAMPAR = varargin{4};
  WEIGHT = varargin{5};
  READOUT = varargin{6};
