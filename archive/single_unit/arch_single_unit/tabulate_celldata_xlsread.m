%% this version assumes summary data will be xls/xlsx files
...
summary_file_listfile = 'summary_file_list';
...
% % % importSummary = importdata('GratingOrientationSummary.dat');
...
% % % maxlocs = find(strcmp('MaxLoc',varnames)); 
% % % if length(maxlocs) > 1 % if there are multiple vars named 'MaxLoc'
% % %     varnames{maxlocs(2)} = 'MaxLoc2'; % rename to avoid duplicate table variables
% % % end
% % % thisProtSummary = table(importSummary.textdata(2:end,1),'VariableNames',importSummary.textdata(1,1)); % add file name strings from first column to table
...
for ind = 1:height(summarydata)
    [~, ~, thisProtSummary] = xlsread(summarydata.summary_file{ind});
    vartitles = thisProtSummary(1,:);
    vartitles = cellfun(@(x) strrep(x,' ',''),vartitles,'UniformOutput',false); % make into table-compatible varnames
    thisProtSummary = cell2table(thisProtSummary(2:end,:),'VariableNames',vartitles); % ignore first row because it is varnames
...
[subRowStart subCol] = find(strcmp(cell_recs,'Subject Name'));
[~, cellCol] = find(strcmp(cell_recs,'Cell #'));
[~, rfAzmCol] = find(strcmp(cell_recs,'Azim Ctr')); % cell rf location relative to vert meridian
[~, rfElvCol] = find(strcmp(cell_recs,'Elev Ctr')); % cell rf location relative to hrz meridian
[~, ofsAzmCol] = find(strcmp(cell_recs,'Azim Ofs')); % screen azimuth offset from the coronal plane
[~, ofsElvCol] = find(strcmp(cell_recs,'Elev Ofs')); % screen elevation offset from the coronal plane
...
for subind = 1:length(unq_subjs)
    ...
    for cellind = 1:length(unq_cells_thissubj)
        ...
        celldata = [celldata; table(thissubj,thiscell,{'_'},{'_'},NaN,NaN,NaN,NaN,'VariableNames',{'recdate','cell','subject','area','rf_azm','rf_elv','ofs_azm','ofs_elv'})];
