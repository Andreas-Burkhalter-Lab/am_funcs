...
%%% 8/30/16 AM edited on msi

%% parameters
clear_workspace = 0; % if true, clear all vars except celldata and files_with_dirs after running
...
protlist = vars_to_show.Properties.VariableNames; % protocol list
...
summaryVarNames = [summaryFileNames(:,1); vars_to_show.Properties.VariableNames'];
...
rspvVarNames = cellfun(@(x)['rspv_',x],vars_to_show(:,1)','UniformOutput',false);
responsiveTable = array2table(false(ncells,size(vars_to_show,1)),'RowNames',celldata.Properties.RowNames,'VariableNames',rspvVarNames);
celldata = [celldata responsiveTable]; 

% for indVar = 1:size(vars_to_show,1)
    
    
% for indCell = 1:ncells
%     for indVar = 1:size(vars_to_show,1)
%         thisvar = vars_to_show{indVar,1};
