%% this version expects that recordings_summary will list one area per subject; future versions expect an area listed in cell_recordings for each cell
...
%         if strcmp(thisprot,'LOOMING_SPEED')
        ...
        elseif length(matchrow) == 1
            colmatch = find(thisrun == prot_type_list{matchrow, protNamesList}); % find protocol for thsi run
