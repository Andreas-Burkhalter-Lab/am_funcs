function [pars] = SF_loggaussfit(means,raw)
...
% ? originally written by Greg DeAngelis for single unit electrical recording tuning curves ?  
% AM adapted for CA imaging; last updated 3/5/18
...
fixed_v0=0; %set this flag to true and set the default value you like to loggaussfunc, then also at bottom of this func.
...
if ~(fixed_v0)
  ...
  A=[]; B=[]; Aeq=[]; Beq=[]; NONLCON=[];
  LB=[0;0;min_x;.00005;.0001];
  UB=[1.5*min_val;1.5*(max_val - min_val);5;5;0.01];
else
  %in general it's pretty quick to do a coarse
  %blind search of the parameter space, so we do one now (only the last 3 params though):
  if ~(fixed_sigma)
    min_err=Inf;
    for i = 0.01:0.02:1      % vn
        for j = .05:05:4    % sigma  for k = .05:.05:2 % sigma
              qtemp = q;
              qtemp(3) = i;
              qtemp(4) = j;
              temp_err = loggauss_err(qtemp);
    ...
    A=[]; B=[]; Aeq=[]; Beq=[]; NONLCON=[];
  LB=[0;0;min_x;.0005;.0001];
  UB=[1.5*min_val;1.5*(max_val - min_val);5;10;0.01];
  else
    min_err=Inf;
    for i=0.01:0.02:1
        qtemp=q;
...
OPTIONS = optimset('MaxIter',5000); %% AM replaced above two lines with this line 10/5/15
