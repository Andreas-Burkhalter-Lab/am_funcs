...
%%%% 4/27/16 AM added escape from endless while loop..... endless loops
%%%%    seem to occur when the cell is not well-tuned for the parameter
%%%%    (function does not fit the data well); in these cases turning on
%%%%    countlimiter may alter results
%%%% 4/15/16 AM edited formatting of output file columns 
...
function SFTuningCurve(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE, skip_trials)

countlimiter = 1; %%% AM added 4/27/16 - toggle on to limit while loops to countlimit cycles
countlimit = 1*10^5.5; %%% AM added 4/27/16 to break endless loop
...
for j = 1:length(unique_stim_type);
    ...
    index = max_i;
    ysearch = pmax{j}.y;
% % %     count = 0; %%% AM added 4/27/16 to break endless loop    
    while (ysearch > (pmin{j}.y + (pmax{j}.y-pmin{j}.y)/2))
% % %         count = count+1; %%% AM added 4/27/16 to break endless loop 
% % %         if countlimiter && count > countlimit; %%% AM added 4/27/16 to break endless loop
% % %             break  %%% AM added 4/27/16 to break endless loop
% % %         end   %%% AM added 4/27/16 to break endless loop
...
%         i = i - 1;
%     end
% %also write out data in form suitable for plotting tuning curve with
% %Origin. 
...
%     fclose(proffid);
    ...
    if strcmp(getenv('COMPUTERNAME'),'ANDREW') %%% Andrew's MSI laptop name
        outfile = 'C:\Users\AM\Documents\Lab\recordings\GratingSF_Summary.dat';
    else
    ...
    if (printflag)
        ...
        %%%% output file
        %%% original line: fprintf(fid, 'FILE\t\t\t PrfOR\t PrfSF\t PrfTF\t RFXctr\t RFYctr\t RFDiam\t GrType\t MaxLoc\t MaxRsp\t MinLoc\t MinRsp\t Spont\t SFMI\t\t AnovaP\t SFDI  \t VarTerm\t MaxX \t MinX \t SFHWHM\t SFHW25\t SFHW20\t SFHW15\t SFHW10\t RsqMn\t PMean\t RsqRaw\t Praw\t\t Chi2\t\t ChiP\t\t Brat\t\t Ampl\t\t Peak\t\t SD\t\t Offst\t\t');
        fprintf(fid, 'FILE\t PrfOR\t PrfSF\t PrfTF\t RFXctr\t RFYctr\t RFDiam\t GrType\t MaxLoc\t MaxRsp\t MinLoc\t MinRsp\t Spont\t SFMI\t AnovaP\t SFDI  \t VarTerm\t MaxX \t MinX \t SFHWHM\t SFHW25\t SFHW20\t SFHW15\t SFHW10\t RsqMn\t PMean\t RsqRaw\t Praw\t Chi2\t ChiP\t Brat\t Ampl\t Peak\t SD\t Offst\t'); %%%% 4/15/16 AM edited 
        fprintf(fid, '\r\n');
        printflag = 0;
    end
    for j = 1:length(unique_stim_type)
