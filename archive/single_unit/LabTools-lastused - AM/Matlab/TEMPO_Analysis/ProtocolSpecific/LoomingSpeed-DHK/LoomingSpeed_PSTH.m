...
function LoomingSpeed_PSTH(data, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, StartEventBin, StopEventBin, PATH, FILE, Protocol);
    ...
    for trial=1:length(speed);
        if (speed(trial) == optimized_stimulus & stim_type(trial) == unique_stim_type(j));
            ...
            elseif j == 2;
                ...
                'Error: More than two Looming types'
            end
            hold on;
        end
    end
...
%    plot(bin_centers,hits2,lines{2})
    title('Optimized Looming Speed PSTH')
end
...
%    plot(bin_centers,normalized_hits2,lines{2})
    title('Optimized Looming Speed PSTH - Normalized')
end
...
if (output == 1)
    ...
    outfile = [BASE_PATH 'ProtocolSpecific\LoomingSpeed-DHK\OptimizedLoomingSpeed_PSTH.dat'];
    printflag = 0;
...
if (output == 1)
    ...
    %write out all relevant parameters to a cumulative text file, GCD 8/08/01
    %write out one line for each stimu_type for each neuron.
    outfile = [BASE_PATH 'ProtocolSpecific\LoomingSpeed-DHK\OptimizedLoomingSpeed_PSTH_Norm.dat'];
    printflag = 0;
    if (exist(outfile, 'file') == 0)    %file does not yet exist
        printflag = 1;
    end
