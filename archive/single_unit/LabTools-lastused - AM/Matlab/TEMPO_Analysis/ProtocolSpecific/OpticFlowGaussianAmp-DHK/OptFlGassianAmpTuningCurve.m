...
%--	GCD, 1/26/00
%-----------------------------------------------------------------------------------------------------------------------
function OptFlGassianAmpTuningCurve(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
amplitude = data.dots_params(DOTS_AP_DIST,:,PATCH1);
...
null_trials = logical( (amplitude == data.one_time_params(NULL_VALUE)) );
unique_speed = munique(amplitude(~null_trials)');

%now, remove trials from amplitude and spike_rates that do not fall between BegTrial and EndTrial
trials = 1:length(amplitude);		% a vector of trial indices
select_trials = ( (trials >= BegTrial) & (trials <= EndTrial) );



plot_x = amplitude(~null_trials & select_trials);
plot_y = spike_rates(~null_trials & select_trials);

figure;
set(gcf,'PaperPosition', [.51 .51 20 27.5], 'Position', [250 50 500 573], 'Name', 'amplitude Tuning Curve');
subplot(2, 1, 2);
hold on;
...
% Compute a amplitude discrimination index analogous to the DDI
...
%now, print out amplitude tuning curve specific stuff
...
%     fclose(proffid);
    ...
    outfile = [BASE_PATH 'ProtocolSpecific\OpticFlowGassianAmp-DHK\GassianAmp_Summary.dat'];
    printflag = 0;
