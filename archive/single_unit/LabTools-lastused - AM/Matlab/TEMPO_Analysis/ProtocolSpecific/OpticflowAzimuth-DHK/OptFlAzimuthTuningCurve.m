...
%--	GCD, 1/26/00
%-----------------------------------------------------------------------------------------------------------------------
function OptFlAzimuthTuningCurve(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
azimuth = data.dots_params(DOTS_AP_AZI,:,PATCH1);
...
null_trials = logical( (azimuth == data.one_time_params(NULL_VALUE)) );
unique_speed = munique(azimuth(~null_trials)');

%now, remove trials from azimuth and spike_rates that do not fall between BegTrial and EndTrial
trials = 1:length(azimuth);		% a vector of trial indices
select_trials = ( (trials >= BegTrial) & (trials <= EndTrial) );



plot_x = azimuth(~null_trials & select_trials);
plot_y = spike_rates(~null_trials & select_trials);

figure;
set(gcf,'PaperPosition', [.51 .51 20 27.5], 'Position', [250 50 500 573], 'Name', 'azimuth Tuning Curve');
subplot(2, 1, 2);
hold on;
...
% Compute a azimuth discrimination index analogous to the DDI
...
%now, print out azimuth tuning curve specific stuff
...
if (output == 1)
    ...
    outfile = [BASE_PATH 'ProtocolSpecific\OpticFlowAzimuth-DHK\Azimuth_Summary.dat'];
    printflag = 0;
