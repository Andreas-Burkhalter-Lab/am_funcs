...
function GratingRFProfile(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE, skip_trials)
...
if (output == 1)
    ...
    if strcmp(getenv('COMPUTERNAME'),'ANDREW') %%% Andrew's MSI laptop name
        outfile = 'C:\Users\AM\Documents\Lab\recordings\RF_Summary.dat';
    else
        outfile = ['C:\LabTools\Matlab\TEMPO_Analysis\ProtocolSpecific\RFMapping\RF_Summary.dat']; %%% original line
    end
    ...
    if (printflag)
        ...
        %%%% output file
        %%% original line: fprintf(fid, 'FILE\t\t\t PrfOR\t PrfSF\t PrfTF\t RFXctr\t RFYctr\t RFDiam\t BaseRate\t Amp\t\t x_width\t y_width\t x_ctr\t y_ctr\t rfplot_x_ctr\t rfplot_y_ctr\t axpect_ratio\t rotation_from_horizontal\t');
    ...
    for j = 1:1
        ...
        %%%% instances of '\t', which added extra unnecessary columns to
        %%%% output file
        %%% original line:  buff = sprintf('%s\t %5.1f\t %5.2f\t %5.3f\t %5.2f\t %5.2f\t %5.2f\t %5.2f\t %5.2f\t %5.2f\t %5.2f\t %5.2f\t %5.2f\t %5.2f\t\t %5.2f\t\t %5.2f\t\t %5.2f\t\t', ...
