%Edited on 1/20/08 to by RLS incorporate a seperate 'Pooling' function for use
...
function Size_PSTH(data, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, StartEventBin, StopEventBin, PATH, FILE, Protocol, Pooling);
...
unique_siz = munique(siz(~null_trials)');
...
for j = 1:length(unique_stim_type);
    ...
    plot_x=siz(~null_trials & select_trials & grating_select);
    ...
    hold on
    for trial=1:length(siz);
        if (siz(trial) == optimized_stimulus & stim_type(trial) == unique_stim_type(j));
            count = count+1;
...
if plots_on == 1;
    ...
    title('Optimized Grating Size PSTH')
end
...
if (output == 1)
    ...
    outfile = [BASE_PATH 'ProtocolSpecific\GratingSize\OptimizedSize_PSTH.dat'];
    printflag = 0;
...
if ~isempty(total_spikes2)
    if (output == 1)
        ...
        %write out all relevant parameters to a cumulative text file, GCD 8/08/01
        %write out one line for each stimu_type for each neuron.
        outfile = [BASE_PATH 'ProtocolSpecific\GratingSize\OptimizedSize_PSTH2.dat'];
        printflag = 0;
        if (exist(outfile, 'file') == 0)    %file does not yet exist
            printflag = 1;
        end
...
if (output == 1)
    ...
    %write out all relevant parameters to a cumulative text file, GCD 8/08/01
    %write out one line for each stimu_type for each neuron.
    outfile = [BASE_PATH 'ProtocolSpecific\GratingSize\OptimizedSize_PSTH_Norm.dat'];
    printflag = 0;
    if (exist(outfile, 'file') == 0)    %file does not yet exist
        printflag = 1;
    end
...
if ~isempty(total_spikes2)
    if (output == 1)
        ...
        %write out all relevant parameters to a cumulative text file, GCD 8/08/01
        %write out one line for each stimu_type for each neuron.
        outfile = [BASE_PATH 'ProtocolSpecific\GratingSize\OptimizedSize_PSTH_Norm2.dat'];
        printflag = 0;
        if (exist(outfile, 'file') == 0)    %file does not yet exist
            printflag = 1;
        end
