%-----------------------------------------------------------------------------------------------------------------------
%-- DirectionTuning_Playback.m -- Play back data from a direction tuning run
%--	GCD, 4/22/01
%-----------------------------------------------------------------------------------------------------------------------
function DirectionTuning_Playback(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);

TEMPO_Defs;		%needed for defines like IN_T1_WIN_CD
ProtocolDefs;	%needed for all protocol specific functions - contains keywords - BJP 1/4/01

DirecTuning_Playback_Loop(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);%, gui_handles);

return;


%----------------------------------------------------------------------------------------------------------------------------------------------------
function DirecTuning_Playback_Loop(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);%, gui_handles);
...
figure(2);
set(gcf,'PaperPosition', [.2 .2 8 10.7], 'Position', [2 55 900 600], 'Name', 'Direction Tuning');
...
select_trials = find(direction >= 0);
...
for i = 1:length(select_trials)
    ...
    while (elap < 3.9)
        ...
        if ( (elap > start_eventbin*event_bin_width) & (elap < stop_eventbin*event_bin_width) )  %the visual stimulus period
            ...
            dir = direction(select_trials(i));
            %if (dir > 180)
            %    dir = dir -360;
            %end
            buff = sprintf('Direction = %d deg',  dir);
    ...
    subplot(h_curve);    
    tempdir = direction(select_trials(1:i));
    %tempdir(tempdir > 180) = tempdir(tempdir > 180) - 360;
    [xx, index] = sort(tempdir);
