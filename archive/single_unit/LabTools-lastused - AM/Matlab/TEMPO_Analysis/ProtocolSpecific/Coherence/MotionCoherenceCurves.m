...
%%% 3/14/16 AM corrected capitalization to make compatible with newer
%%% matlab versions
%%%% 7/29/16 AM added conditional when writing outfile to write into the correct
%%%%    directory if on a specific computer; functionality should be
%%%%    unchanged if using on any lab computer
%%%% 7/29/16 AM edited formatting of output file columns 
...
function MotionCoherenceCurves(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE, skip_trials)
...
yl = ylim; %%% 3/14/16 AM changed from 'YLim'
ylim([0 yl(2)]);	% set the lower limit of the Y axis to zero %%% 3/14/16 AM changed from 'YLim'
xlabel('log(percent coherence)'); %%% 3/14/16 AM changed from 'XLabel'
ylabel('Response (spikes/sec)'); %%% 3/14/16 AM changed from 'YLabel'
...
if (output == 1)
    ...
    %%%% 7/29/16 AM added conditional below to write into the correct
    ...
    if strcmp(getenv('COMPUTERNAME'),'ANDREW') %%% Andrew's MSI laptop name
        outfile = 'C:\Users\AM\Documents\Lab\recordings\Coherence_Summary.dat';
    else
    ...
    fid = fopen(outfile, 'a');
    if (printflag)
        %%%% 7/29/16 AM edited following line - removed all repeated
