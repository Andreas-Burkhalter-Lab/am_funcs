%-----------------------------------------------------------------------------------------------------------------------
%-- SpeedTuningCurveGaussian.m -- Plots a speed tuning curve, fits with a regular Gaussian.
%--	GCD, 3/10/05
%-----------------------------------------------------------------------------------------------------------------------
function OptFlSpeedTuningCurveGaussian(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
figure;
set(gcf,'PaperPosition', [.2 .2 8 10.7], 'Position', [250 50 500 573], 'Name', 'Speed Tuning Curve -- Gaussian Fit');
subplot(2, 1, 2);
...
PrintSpeedDataGaussian(plot_x, plot_y, pmax, pmin, px, null_rate, pars, fit_err, p_value, avg_resp, stats1, stats2,SDI,chi2,chiP);
...
if output == 1
    ...
    FILEOUT = [FILE(1:i) 'spd_Gauss'];
    
    fileid = [PATHOUT FILEOUT]
    fwriteid = fopen(fileid, 'w')
    
    %fprintf(fwriteid, ' Max Speed	Max Resp Min Speed	Min Resp	Ave Resp	Spont Resp	Curve ANOVA	Fit q(1)	Fit q(2)	Fit q(3)	Fit q(4)	Pref Dir	Pref Speed	Mapped Pref H Disp	RF X-Ctr	RF Y-Ctr	RF Diam\n');  
    
    fprintf(fwriteid, '%.6f	%.6f	%.6f	%.6f	%.6f	%.6f	%.6f	%.6f	%.6f	%.6f	%.6f	%.2f	%.2f	%.2f	%.2f	%.2f	%.2f\n', pmax.x, pmax.y, pmin.x, pmin.y, avg_resp, null_rate, p_value, pars(1), pars(2), pars(3), pars(4), data.one_time_params(PREFERRED_DIRECTION), data.one_time_params(PREFERRED_SPEED), data.one_time_params(PREFERRED_HDISP), data.one_time_params(RF_XCTR), data.one_time_params(RF_YCTR), data.one_time_params(RF_DIAMETER));
    
    fclose(fwriteid);
    
    %also write out data in form suitable for plotting tuning curve with Origin.
    FILEOUT2 = [FILE(1:i) 'spd_curv_Gauss'];
    ...
    buff = sprintf('%s\t %6.1f\t %6.2f\t %6.3f\t %6.2f\t %6.2f\t %6.2f\t %5.1f\t %6.2f\t %6.3f\t %6.2f\t %6.3f\t %6.2f\t %6.2f\t %6.3f\t %10.8f\t %6.3f\t %10.8f\t %6.3f\t %10.8f\t %6.4f\t %6.3f\t %6.3f\t %6.3f\t %10.7f\t %10.7f\t %8.5f\t %10.8f\t %6d\t %6d\t %6.4f\t', ...
    ...
    outfile = [BASE_PATH 'ProtocolSpecific\OpticFlowSpeed-DHK\OpticFlowSpeedTuningSummary_Gaussian.dat'];
    printflag = 0;
    ...
    fid = fopen(outfile, 'a');
    if (printflag)
        fprintf(fid, 'FILE\t\t PrDir\t PrSpd\t PrHDsp\t RFX\t RFY\t RFDiam\t ZerSpd\t ZerRsp\t MaxSpd\t MaxRsp\t MinSpd\t MinRsp\t Spont\t SpdMI\t AnovaP\t\t Rmeans\t Pmeans\t\t Rraw\t Praw\t\t SDI\t VarTrm\t R0\t K\t ctr\t\t sigma\t\t Chi2\t\t ChiP\t\t Npts\t Npars\t FitErr\t');
        fprintf(fid, '\r\n');
    end
    fprintf(fid, '%s', buff);
    fprintf(fid, '\r\n');
