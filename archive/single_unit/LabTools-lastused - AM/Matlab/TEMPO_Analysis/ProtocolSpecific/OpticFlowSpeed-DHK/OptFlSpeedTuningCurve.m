...
function OptFlSpeedTuningCurve(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE, skip_trials)
...
%     fclose(proffid);
    ...
    if strcmp(getenv('COMPUTERNAME'),'ANDREW') %%% Andrew's MSI laptop name
        outfile = 'C:\Users\AM\Documents\Lab\recordings\OpticFlowSpeed_Summary.dat';
    else
        %%%%%% 3/14/16 AM updated summary file path below; was previously C:\LabTools\Matlab\TEMPO_Analysis\ProtocolSpecific\OpticFlowSpeed-DHK\OpticFlowSpeed_Summary.dat
        outfile = [BASE_PATH 'ProtocolSpecific\OpticFlowSpeed-DHK\OpticFlowSpeed_Summary.dat'];
    end
    ...
    if (printflag)
        ...
        %%%% output file
        %%% original line: fprintf(fid, 'FILE\t\t\t PrfOR\t PrfSF\t PrfTF\t RFXctr\t RFYctr\t RFDiam\t     MaxLoc\t MaxRsp\t MinLoc\t MinRsp\t Spont\t SMI\t\t AnovaP\t SDI\t\t VarTerm\t MaxX \t MinX \t HWHML\t HWHMR\t RsqMn\t PMean\t RsqRaw\t Praw\t\t Chi2\t\t ChiP\t\t Brat\t\t Ampl\t\t Peak\t\t SD\t\t Offst\t\t');
