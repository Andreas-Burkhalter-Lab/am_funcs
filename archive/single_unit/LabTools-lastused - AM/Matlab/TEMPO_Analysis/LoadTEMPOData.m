...
%%% 9/27/16 AM edited call to ReadTEMPOLog to include 'looming_params'
%%% output, added looming_params as field to data struct

function	[return_value, good_data, bad_data, newChans] = LoadTEMPOData(PATH,FILE)

global binned_ced_spikes good_data listtext num_recorded_spike_channels pool_data1 pool_data2 pool_size sample_trials pool_trials pool_file bin_size window_size
...
[gratings_params, revcorr_params, dots_params, moog_params, targ_params, cue_params, misc_params, one_time_params, eye_calib_params, obj_params, bar_params, bkgnd_params, neuron_params, looming_params] =...
    ReadTEMPOLog(logfile, num_trials);   %%% 9/27/16 AM edited to include 'looming_params' output
...
%deal with occasional cases where there are more epochs in the events
%database than in the other databases
n_ev_trials = size(temp3,3);
n_sp_trials = size(temp2,3);
n_ey_trials = size(temp1,3);
if (n_ev_trials > n_sp_trials)
    n_ev_trials = n_sp_trials;
    disp('WARNING:num_events_trials > num_spikes_trials.  Correcting...');
end
if (n_ev_trials > n_ey_trials)
    n_ev_trials = n_ey_trials;
    disp('WARNING:num_events_trials > num_eye_trials.  Correcting...');
end
% n_ev_trials

all_trials = 1:n_ev_trials;
good_trials = find(temp3(:,:,1:n_ev_trials) == SUCCESS_CD);
...
% good_data.dots_draw_params = dots_draw_params;
% good_data.dots_ap_params = dots_ap_params;
good_data.looming_params = looming_params;
% good_data.enable_params = enable_params;
% good_data.stim_params = stim_params;
% good_data.eye_params = eye_params;
...
% if ~isempty(dots_draw_params)
%     good_data.dots_draw_params = dots_draw_params(:,good_trials,:);
% end
% if ~isempty(dots_ap_params)
%     good_data.dots_ap_params = dots_ap_params(:,good_trials,:);
% end
if ~isempty(looming_params)
    good_data.looming_params = looming_params(:,good_trials,:);
end
% if ~isempty(enable_params)
%     good_data.enable_params = enable_params(:,good_trials,:);
% end
% if ~isempty(stim_params)
%     good_data.stim_params = stim_params(:,good_trials,:);
% end
% if ~isempty(eye_params)
%     good_data.eye_params = eye_params(:,good_trials,:);
% end
...
if (~isempty(gratings_params))
    good_data.gratings_params = gratings_params(:, good_trials);
    bad_data.gratings_params = gratings_params(:, bad_trials);
end
...
% bad_data.dots_draw_params = dots_draw_params;
% bad_data.dots_ap_params = dots_ap_params;
bad_data.looming_params = looming_params;
% bad_data.enable_params = enable_params;
% bad_data.stim_params = stim_params;
% bad_data.eye_params = eye_params;
...
% if ~isempty(dots_draw_params)
%     bad_data.dots_draw_params = dots_draw_params(:,good_trials,:);
% end
% if ~isempty(dots_ap_params)
%     bad_data.dots_ap_params = dots_ap_params(:,good_trials,:);
% end
if ~isempty(looming_params)
    bad_data.looming_params = looming_params(:,good_trials,:);
end
% if ~isempty(enable_params)
%     bad_data.enable_params = enable_params(:,good_trials,:);
% end
% if ~isempty(stim_params)
%     bad_data.stim_params = stim_params(:,good_trials,:);
% end
% if ~isempty(eye_params)
%     bad_data.eye_params = eye_params(:,good_trials,:);
% end
