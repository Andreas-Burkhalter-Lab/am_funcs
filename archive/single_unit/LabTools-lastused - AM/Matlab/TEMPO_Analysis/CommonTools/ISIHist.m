...
for cond = 1: num_conditions
    ...
    for trial = 1: num_reps
 	    zero_bin = [];
        zero_bin2 = [];
        spikes1 = find (data.spike_data(SpikeChan, StartEventBin(reps(trial)) + StartOffset: StartEventBin(reps(trial)) + StartOffset + AnalInterval ,reps(trial) ) >= 1) ;
 	    spikes2 = find (data.spike_data(SpikeChan2, StartEventBin(reps(trial)) + StartOffset: StartEventBin(reps(trial)) + StartOffset + AnalInterval ,reps(trial) ) >= 1);
   
        zero_bin = find(data.spike_data(SpikeChan,StartEventBin(reps(trial)) + StartOffset: StartEventBin(reps(trial)) + StartOffset + AnalInterval, reps(trial)  ) > 1);
        zero_bin2 = find(data.spike_data(SpikeChan2,StartEventBin(reps(trial)) + StartOffset: StartEventBin(reps(trial)) + StartOffset + AnalInterval, reps(trial)  ) > 1);

        isi{cond} = [isi{cond} diff(spikes1)]; 
      	isi2{cond} = [isi2{cond} diff(spikes2)];
            %for multiple spikes in a bin, place number of spikes - 1 counts in
        %bin 0

        if ~isempty(zero_bin)
            zero_bin = data.spike_data(SpikeChan,zero_bin +StartEventBin(reps(trial)) + StartOffset -1 , reps(trial)  );
            zero_bin = zero_bin - 1;
            isi{cond} = [isi{cond} zeros(1,sum(zero_bin) )];
        end   
        if ~isempty(zero_bin2)
            zero_bin2 = data.spike_data(SpikeChan2,zero_bin2 +StartEventBin(reps(trial)) + StartOffset -1 , reps(trial)  );
            zero_bin2 = zero_bin2 - 1;
            isi2{cond} = [isi2{cond} zeros(1,sum(zero_bin2) )];
        end   
