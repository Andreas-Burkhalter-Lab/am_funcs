...
function Speed_PSTH(data, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, StartEventBin, StopEventBin, PATH, FILE, Protocol);
...
%    plot(bin_centers,hits2,lines{2})
    title('Optimized Grating Speed PSTH')
end
...
%    plot(bin_centers,normalized_hits2,lines{2})
    title('Optimized Grating Speed PSTH - Normalized')
end
...
if (output == 1)
    ...
    outfile = [BASE_PATH 'ProtocolSpecific\SpeedTuning\OptimizedSpeed_PSTH.dat'];
    printflag = 0;
...
if (output == 1)
    ...
    %write out all relevant parameters to a cumulative text file, GCD 8/08/01
    %write out one line for each stimu_type for each neuron.
    outfile = [BASE_PATH 'ProtocolSpecific\SpeedTuning\OptimizedSpeed_PSTH_Norm.dat'];
    printflag = 0;
    if (exist(outfile, 'file') == 0)    %file does not yet exist
        printflag = 1;
    end
