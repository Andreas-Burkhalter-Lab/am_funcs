function [pars] = speed_loggaussfit(means,raw)
% speed_loggaussfit fits a Gaussian function to the natural log of the data with an offset using Simplex algorithm.
...
fixed_v0=0; %set this flag to true and set the default value you like to speed_loggaussfunc, then also at bottom of this func.
...
if ~(fixed_v0)
  ...
  for i = .1:.1:10      % vn
      for j = .05:05:8    % sigma
          for k = .005:.005:0.1 % v0
              ...
              qtemp(4) = j;
              qtemp(5) = k;
              temp_err = speed_loggauss_err(qtemp);
              if (temp_err < min_err)
                  min_err = temp_err;
                  q(3) = i;
                  q(4) = j;
  ...
  A=[]; B=[]; Aeq=[]; Beq=[]; NONLCON=[];
  LB=[0;0;min_x;.1;.001];
  UB=[1.5*min_val;1.5*(max_val - min_val);256;max_x;0.1];
else
  %in general it's pretty quick to do a coarse
  %blind search of the parameter space, so we do one now (only the last 3 params though):
  if ~(fixed_sigma)
...
OPTIONS = optimset('MaxIter', 5000); %Ji inset this line to replace front 2 lines
...
for j=1:N_reps
    ...
    testpars{j} = fmincon('speed_loggauss_err',temp_q,A,B,Aeq,Beq,LB,UB, NONLCON, OPTIONS);
    err(j) = speed_loggauss_err(testpars{j});
end
...
if (fixed_v0)
    if (fixed_sigma)
        pars(4)=1.22; %set this value also in speed_loggaussfunc
    end
    pars(5)=0.01; %set this value also in speed_loggaussfunc
end
return;