...
%     plot_y = spike_rates(~null_trials & select_trials);
    ...
    plot_x=speed(ind_laser1);
    plot_y=spike_rates(ind_laser1);
    
    hold on;
    [px, py, perr] = PlotTuningCurve(plot_x', plot_y', 'ro', 'r-', 1, 0);
    ...
    p_value = spk_anova(plot_y, plot_x, px);
    avg_resp = mean(plot_y);
    PrintSpeedData2(p_value, base_rate, amplitude, peak, st_dev, log_offset, max_rate, stats1, stats2, chi2, chiP, SDI, HWHMR, HWHML);
    SMI = Compute_ModIndex(plot_x, plot_y, null_resp);
    if (output == 1)
        i = size(PATH,2) - 1;
        while PATH(i) ~='\'	%Analysis directory is one branch below Raw Data Dir
