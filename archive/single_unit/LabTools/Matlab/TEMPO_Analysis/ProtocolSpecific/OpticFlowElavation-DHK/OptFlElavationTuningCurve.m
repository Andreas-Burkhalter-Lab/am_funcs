%-----------------------------------------------------------------------------------------------------------------------
%-- SpeedTuningCurve.m -- Plots a elavation tuning curve.
%--	GCD, 1/26/00
%-----------------------------------------------------------------------------------------------------------------------
function OptFlElavationTuningCurve(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
elavation = data.dots_params(DOTS_AP_ELE,:,PATCH1);
...
null_trials = logical( (elavation == data.one_time_params(NULL_VALUE)) );
unique_speed = munique(elavation(~null_trials)');

%now, remove trials from elavation and spike_rates that do not fall between BegTrial and EndTrial
trials = 1:length(elavation);		% a vector of trial indices
select_trials = ( (trials >= BegTrial) & (trials <= EndTrial) );



plot_x = elavation(~null_trials & select_trials);
plot_y = spike_rates(~null_trials & select_trials);

figure;
set(gcf,'PaperPosition', [.51 .51 20 27.5], 'Position', [250 50 500 573], 'Name', 'elavation Tuning Curve');
subplot(2, 1, 2);
hold on;
...
% Compute a elavation discrimination index analogous to the DDI
...
xlabel('elavation of Motion (deg/sec)');
...
%now, print out elavation tuning curve specific stuff
...
if (output == 1)
    ...
    outfile = [BASE_PATH 'ProtocolSpecific\OpticFlowElavation-DHK\Elevation_Summary.dat'];
    printflag = 0;
