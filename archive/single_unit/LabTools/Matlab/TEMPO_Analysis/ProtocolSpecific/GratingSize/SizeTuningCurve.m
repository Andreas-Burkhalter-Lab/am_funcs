...
if length(intersect(unique(data.laser_values),[0 1]))==1
    ...
    for j = 1:length(unique_stim_type);
        ...
        y1 = ylim; %JI change YLim to ylim 10/5/2015
        ylim([0 y1(2)]); %set the lower limit of the Y axis to zero %JI change YLim to ylim 10/5/2015
    ...
    if (output == 1)
        ...
        fid = fopen(outfile, 'a');
        if (printflag)
            fprintf(fid, 'FILE\t\t\t PrfOR\t PrfSF\t PrfTF\t RFXctr\t RFYctr\t RFDiam\t GrType\t Spont\t SI\t\t AnovaP\t SDI\t\t VarTerm\t MaxX \t MinX \t RsqMn\t PMean\t RsqRaw\t Praw\t\t ErfChi2\t ErfChiP\t DiffErfChi2\t DiffErfChiP\t Ke\t\t a\t\t Ki\t\t\t b+a\t\t DC_off\t Halfmax_slope\t F_SI\t P_SI\t');
            fprintf(fid, '\r\n');
            printflag = 0;
        end
        for j = 1:length(unique_stim_type)
...
%         plot_y = spike_rates(select_trials & grating_select);
    ...
    subplot(3, 1, 2);
    PrintSizeData2(p_value, Ke, a, Ki, b_plus_a, DC, stats_mean, stats_raw, OptSize, SI, SDI, Erf_chi2, Erf_chiP, Diff_Erf_chi2, Diff_Erf_chiP);
