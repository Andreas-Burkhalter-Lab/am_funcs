...
%% DHK - 2 Laser Conditon -> one experiments
    ...
    if (output == 1)
        ...
        fid = fopen(outfile, 'a');
        if (printflag)
            fprintf(fid, 'FILE\t\t\t PrfOR\t PrfSF\t PrfTF\t RFXctr\t RFYctr\t RFDiam\t GrType\t MaxLoc\t MaxRsp\t MinLoc\t MinRsp\t Spont\t TFMI\t\t AnovaP\t TFDI  \t VarTerm\t MaxX \t MinX \t TFHWHM\t RsqMn\t PMean\t RsqRaw\t Praw\t\t Chi2\t\t ChiP\t\t Brat\t\t Ampl\t\t Peak\t\t SD\t\t Offst\t\t');
            fprintf(fid, '\r\n');
            printflag = 0;
        end
        for j = 1:length(unique_stim_type)
            buff = sprintf('%s\t %5.1f\t %5.2f\t %5.3f\t %5.2f\t %5.2f\t %5.2f\t %5.1f\t %5.4f\t %5.2f\t %5.4f\t %5.2f\t %5.2f\t %5.3f\t %10.8f\t %5.3f\t %6.3f\t %5.2f\t %5.2f\t %7.4f\t %6.4f\t %10.8f\t %6.4f\t %10.8f\t %6.4f\t %10.8f\t %5.2f\t %5.2f\t %6.4f\t %6.4f\t %6.4f\t', ...
                ...
                unique_stim_type(j), pmax{j}.x, pmax{j}.y, pmin{j}.x, pmin{j}.y, null_rate, TFMI(j), p_value(j), TFDI(j), var_term(j), max(unique_tf), min(unique_tf), tf_width_halfmax(j), stats1(j,1), stats1(j,3), stats2(j,1), stats2(j,3), chi2(j), chiP(j), base_rate(j), amplitude(j), peak(j), st_dev(j), log_offset(j) );
            fprintf(fid, '%s', buff);
            fprintf(fid, '\r\n');
        end
        fclose(fid);
...
elseif length(intersect(unique(data.laser_values),[0 1]))==2
    % now as we have two conditions, split the data into 2 groups
    tf = data.gratings_params(GRAT_TEMPORAL_FREQ,:,PATCH1);
    ...
    select_trials0=select_trials(ind_laser0);
    ...
    for j = 1:length(unique_stim_type)
        ...
        plot_x=tf(~null_trials(ind_laser0) & select_trials(ind_laser0) & grating_select(ind_laser0));
        plot_y=spike_rates(~null_trials(ind_laser0) & select_trials(ind_laser0) & grating_select(ind_laser0));
        ...
        %rate to plot
        null_x = [min(px) max(px)];
        null_resp = data.spike_rates(SpikeChan, null_trials(ind_laser0) & select_trials(ind_laser0));
        null_rate = mean(null_resp);
        null_y = [null_rate null_rate];
        hold on;
        plot(null_x, null_y, 'k--');
...
% %     PrintTemporalFreqData(p_value, base_rate, amplitude, peak, st_dev, log_offset, max_rate, stats1, stats2, chi2, chiP, TFDI, tf_width_halfmax);
    ...
    for j = 1:length(unique_stim_type)
        ...
        plot_x=tf(~null_trials(ind_laser1) & select_trials(ind_laser1) & grating_select(ind_laser1));
        plot_y=spike_rates(~null_trials(ind_laser1) & select_trials(ind_laser1) & grating_select(ind_laser1));
