...
function OptFlContrast_PSTH(data, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, StartEventBin, StopEventBin, PATH, FILE, Protocol, Pooling);
...
contrast = data.dots_params(DOTS_RGBLUM,:,PATCH1);
...
for j = 1:length(unique_stim_type);
    ...
    for trial=1:length(contrast);
        if (contrast(trial) == optimized_stimulus & stim_type(trial) == unique_stim_type(j));
            count = count+1;
...
if plots_on == 1;
    figure;
    set(gcf,'PaperPosition', [.51 .51 20 27.5], 'Position', [450 50 500 573], 'Name', 'Optimized Contrast PSTH');
    hold on
    plot(bin_centers,hits1,lines{1})
    if ~isempty(total_spikes2)
        plot(bin_centers,hits2,lines{2})
    end
    title('Optimized Optic Flow Contrast PSTH')
end
...
if plots_on == 1;
    figure;
    set(gcf,'PaperPosition', [.51 .51 20 27.5], 'Position', [450 50 500 573], 'Name', 'Optimized Contrast PSTH (Normalized)');
    hold on
    plot(bin_centers,normalized_hits1,lines{1})
    if ~isempty(total_spikes2)
        plot(bin_centers,normalized_hits2,lines{2})
    end
    title('Optimized Optic Flow Contrast PSTH - Normalized')
end
...
if (output == 1)
    ...
    outfile = [BASE_PATH 'ProtocolSpecific\OpticFlowContrast-DHK\OptimizedContrast_PSTH.dat'];
    printflag = 0;
...
if ~isempty(total_spikes2)
    if (output == 1)
        ...
        %write out all relevant parameters to a cumulative text file, GCD 8/08/01
        %write out one line for each stimu_type for each neuron.
        outfile = [BASE_PATH 'ProtocolSpecific\OpticFlowContrast-DHK\OptimizedContrast_PSTH2.dat'];
        printflag = 0;
        if (exist(outfile, 'file') == 0)    %file does not yet exist
            printflag = 1;
        end
...
if (output == 1)
    ...
    %write out all relevant parameters to a cumulative text file, GCD 8/08/01
    %write out one line for each stimu_type for each neuron.
    outfile = [BASE_PATH 'ProtocolSpecific\OpticFlowContrast-DHK\OptimizedContrast_PSTH_Norm.dat'];
    printflag = 0;
    if (exist(outfile, 'file') == 0)    %file does not yet exist
        printflag = 1;
    end
...
if ~isempty(total_spikes2)
    if (output == 1)
        ...
        %write out all relevant parameters to a cumulative text file, GCD 8/08/01
        %write out one line for each stimu_type for each neuron.
        outfile = [BASE_PATH 'ProtocolSpecific\OpticFlowContrast-DHK\OptimizedContrast_PSTH_Norm2.dat'];
        printflag = 0;
        if (exist(outfile, 'file') == 0)    %file does not yet exist
            printflag = 1;
        end
