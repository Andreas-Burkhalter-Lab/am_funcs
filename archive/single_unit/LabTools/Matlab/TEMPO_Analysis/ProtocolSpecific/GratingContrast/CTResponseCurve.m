...
%         plot_y=spike_rates(~null_trials & select_trials & grating_select);
    ...
    output = 1;
    if (output == 1)outfile = [BASE_PATH 'ProtocolSpecific\GratingContrast\GratingContrast_Summary.dat'];
        printflag = 0;
        if (exist(outfile, 'file') == 0)    %file does not yet exist
            printflag = 1;
        end
...
%         plot_y=spike_rates(~null_trials & select_trials & grating_select);
    ...
    subplot(3, 1, 2);
    PrintGratingContrastData2(p_value, Rmax, C50, n, DC_offset, max_rate, stats1, stats2, chi2, chiP, CDI, CMI);
