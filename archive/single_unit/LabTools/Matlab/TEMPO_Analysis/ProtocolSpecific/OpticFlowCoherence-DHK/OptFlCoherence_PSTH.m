...
function OptFlCoherence_PSTH(data, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, StartEventBin, StopEventBin, PATH, FILE, Protocol);
...
for j = 1:length(unique_direc);
    ...
    grating_select = logical( (direc == unique_direc(j)) );
    
    plot_x=coher(~null_trials & select_trials & grating_select);
...
if plots_on == 1;
    figure;
    set(gcf,'PaperPosition', [.51 .51 20 27.5], 'Position', [450 50 500 573], 'Name', 'Optimized Coherence PSTH');
    hold on
    plot(bin_centers,hits1,lines{1})
    plot(bin_centers,hits2,lines{2})
    title('Optimized Grating Coherence PSTH')
end
...
if plots_on == 1;
    figure;
    set(gcf,'PaperPosition', [.51 .51 20 27.5], 'Position', [450 50 500 573], 'Name', 'Optimized Coherence PSTH (Normalized)');
    hold on
    plot(bin_centers,normalized_hits1,lines{1})
    plot(bin_centers,normalized_hits2,lines{2})
    title('Optimized Grating Coherence PSTH - Normalized')
end
...
if (output == 1)
    ...
    outfile = [BASE_PATH 'ProtocolSpecific\OpticFlowCoherence-DHK\OptimizedOpticFlowCoherence_PSTH.dat'];
    printflag = 0;
...
if (output == 1)
    ...
    %write out all relevant parameters to a cumulative text file, GCD 8/08/01
    %write out one line for each stimu_type for each neuron.
    outfile = [BASE_PATH 'ProtocolSpecific\OpticFlowCoherence-DHK\OptimizedOpticFlowCoherence_PSTH2.dat'];
    printflag = 0;
    if (exist(outfile, 'file') == 0)    %file does not yet exist
        printflag = 1;
    end
...
if (output == 1)
    ...
    %write out all relevant parameters to a cumulative text file, GCD 8/08/01
    %write out one line for each stimu_type for each neuron.
    outfile = [BASE_PATH 'ProtocolSpecific\OpticFlowCoherence-DHK\OptimizedOpticFlowCoherence_PSTH_Norm.dat'];
    printflag = 0;
    if (exist(outfile, 'file') == 0)    %file does not yet exist
        printflag = 1;
    end
...
if (output == 1)
    ...
    %write out all relevant parameters to a cumulative text file, GCD 8/08/01
    %write out one line for each stimu_type for each neuron.
    outfile = [BASE_PATH 'ProtocolSpecific\OpticFlowCoherence-DHK\OptimizedOpticFlowCoherence_PSTH_Norm2.dat'];
    printflag = 0;
    if (exist(outfile, 'file') == 0)    %file does not yet exist
        printflag = 1;
    end
