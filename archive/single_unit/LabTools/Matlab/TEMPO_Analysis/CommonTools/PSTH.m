...
%think. 4/4/03
%06/16/06 Edited program to remove all references to SpikeChan2 for same
%reasons as the file PlotRasters.m CMA
...
for cond = 1: num_conditions
    ...
    trial_spike_rates{cond} = zeros(1, num_reps(cond));
    trial_spike_rates2{cond} = zeros(1, num_reps(cond));
    hist_data{cond} = zeros(num_reps(cond), AnalInterval + PostAnalInterval);
    hist_dataB{cond} = zeros(num_reps(cond), AnalInterval + PostAnalInterval);
    ...
    case LFP_DB
        % initialize for first trial     
        start_bin = StartEventBin(reps(1)) + StartOffset;
        raw_signal = interp( data.lfp_data(SpikeChan,:, reps(1) ), 2);  
        raw_signal = raw_signal(  (start_bin ) : (start_bin + AnalInterval - 1 + PostAnalInterval) ); 
        hist_data{cond}(1,:) = (raw_signal.*raw_signal);
        for i = 1:length(raw_signal)/bin_width
            hist_data2(cond,i) = mean(hist_data{cond}(1, (i-1)*bin_width+ 1:i*bin_width) );
            hist_data3{cond}(1,i) = mean(hist_data{cond}(1, (i-1)*bin_width+ 1:i*bin_width) );
        end
        for trial = 2: num_reps(cond)
            start_bin = StartEventBin(reps(trial)) + StartOffset;
            raw_signal = interp( data.lfp_data(SpikeChan,:, reps(1) ), 2);  
            raw_signal = raw_signal(  start_bin  : (start_bin + AnalInterval - 1 + PostAnalInterval) ); 
            hist_data{cond}(trial,:) = (raw_signal .*raw_signal);
            for i = 1:length(raw_signal)/bin_width
                hist_data2(cond,i) = hist_data2(cond,i) + mean(hist_data{cond}(trial, (i-1)*bin_width+ 1:i*bin_width) );
                hist_data3{cond}(trial,i) = mean(hist_data{cond}(trial, (i-1)*bin_width+ 1:i*bin_width) );
            end
        end
        %take mean of squared signal
        hist_data2(cond,:) = hist_data2(cond,:) / num_reps(cond);
        %Take square root of mean squared signal
        hist_data2(cond,:) = 10000*hist_data2(cond,:).^0.5;
        scale(cond) = max(hist_data2(cond,:)  )  ;
        bins = bin_width/2:bin_width:bin_width*length(hist_data2(cond,:));
    end
    ...
    case SPIKE_DB
        ...
        hist_dataB{cond}(1,:)  = raw_spikes(1,:);
        [bins, counts] = SpikeBinner(raw_spikes(1,:), 1, bin_width, 0);
        ...
        for trial = 2: num_reps(cond)
            ...
            hist_dataB{cond}(trial,:) = raw_spikes(trial,:);
            [bins, counts] = SpikeBinner(raw_spikes(trial,:), 1, bin_width, 0);        
            hist_data3B{cond}(trial,:) = (counts'/(bins(2)/1000-bins(1)/1000));   
        end
        [bins, counts] = SpikeBinner(sum(hist_dataB{cond}, 1) , 1, bin_width, 0);
    ...
    case LFP_DB
        % initialize for first trial     
        start_bin = StartEventBin(reps(1)) + StartOffset;
        raw_signal = interp( data.lfp_data(SpikeChan2,:, reps(1) ), 2);  
        raw_signal = raw_signal(  start_bin  : (start_bin + AnalInterval - 1 + PostAnalInterval) ); 
        hist_dataB{cond}(1,:) = (raw_signal .*raw_signal);
        for i = 1:length(raw_signal)/bin_width
            hist_data2B(cond,i) = mean(hist_dataB{cond}(1, (i-1)*bin_width+ 1:i*bin_width) );
            hist_data3B{cond}(1,i) = mean(hist_dataB{cond}(1, (i-1)*bin_width+ 1:i*bin_width) );
        end
        for trial = 2: num_reps(cond)
            start_bin = StartEventBin(reps(trial)) + StartOffset;
            raw_signal = interp( data.lfp_data(SpikeChan2,:, reps(1) ), 2);  
            raw_signal = raw_signal(  (start_bin ) : (start_bin + AnalInterval - 1 + PostAnalInterval) ); 
            hist_dataB{cond}(trial,:) = (raw_signal .*raw_signal);
            for i = 1:length(raw_signal)/bin_width
                hist_data2B(cond,i) = hist_data2B(cond,i) + mean(hist_dataB{cond}(trial, (i-1)*bin_width+ 1:i*bin_width) );
                hist_data3B{cond}(trial,i) = mean(hist_dataB{cond}(trial, (i-1)*bin_width+ 1:i*bin_width) );
            end        
        end
        ...
        %Take square root of mean squared signal
        hist_data2B(cond,:) = 10000*hist_data2B(cond,:).^0.5;
        scaleB(cond) = max(hist_data2B(cond,:)  )  ;
        bins = bin_width/2:bin_width:bin_width*length(hist_data2(cond,:));
    ...
    if Protocol > 200
        switch Protocol
            ...
            case FIX_VARY_HISTORY
                if cond < 6  % cond >= 6 == apertured conditions
                   spont_condition = 1;
                else
                   spont_condition = 6; %cond 6 apertured mask control
                end   
            case FIX_VARY_BACKCOLOR
                if cond < 4
                   spont_condition = 1;
                else
                   spont_condition = 4; %cond 4 apertured mask control
                end   
            otherwise
                spont_condition = 1;
        end    
...
for cond = 1:num_conditions
   ...
   for segment = 1:length(mini_segment_start_times{cond})
        selected_spikes1(cond) = selected_spikes1(cond) + sum(sum(hist_data{cond}(:,mini_segment_start_times{cond}(segment): mini_segment_end_times{cond}(segment)   ), 1 ) );    
        selected_spikes2(cond) = selected_spikes2(cond) + sum(sum(hist_dataB{cond}(:, mini_segment_start_times{cond}(segment): mini_segment_end_times{cond}(segment)   ), 1 ) );    
        ...
        for trial = 1:num_reps(cond)
            trial_spike_rates{cond}(trial) = trial_spike_rates{cond}(trial) + sum(hist_data{cond}(trial,mini_segment_start_times{cond}(segment): mini_segment_end_times{cond}(segment)   ), 2 ) ;    
            trial_spike_rates2{cond}(trial) = trial_spike_rates2{cond}(trial) + sum(sum(hist_dataB{cond}(trial, mini_segment_start_times{cond}(segment): mini_segment_end_times{cond}(segment)   ), 2 ) );    
        end         
   end 
    trial_spike_rates{cond} = trial_spike_rates{cond} / (selected_time / 1000);
    trial_spike_rates2{cond} = trial_spike_rates2{cond} / (selected_time / 1000);
   ...
   z_dist1 = trial_spike_rates{cond};
   z_dist2 = trial_spike_rates2{cond};
   z_dist1 = (z_dist1 - mean(z_dist1))/std(z_dist1);
   z_dist2 = (z_dist2 - mean(z_dist2))/std(z_dist2);
   
   %high-pass filter the Z-scored responses
   Filt_Zspikes1 = FIR_Filter(z_dist1, 20, 100, 'high', 20, 0);
   Filt_Zspikes2 = FIR_Filter(z_dist2, 20, 100, 'high', 20, 0);
   Filt_Zspikes1 = Filt_Zspikes1(1:length(Filt_Zspikes1));
   Filt_Zspikes2 = Filt_Zspikes2(1:length(Filt_Zspikes2));
   
   %[Z_R,Z_P]=CORRCOEF(Z_Spikes1,Z_Spikes2);
   [Z_R,Z_P]=CORRCOEF(Filt_Zspikes1,Filt_Zspikes2);
   
   NoiseCorrCoef(cond) = Z_R(2);
   NoiseCorrPValue(cond) = Z_P(2);
