%% the output cortable of this version delete pairs which don't match criteria
%% rather than keeping and marking the excluded pairs, like later versions

%%% plot umdist against corcoef
% cortable_out = pairwise_cor_plotting(cortable)
%%%%% updated 2020-02-23 on thermaltake
...
pars.plot_all_quants_together = field_default(pars,'plot_all_quants_together',0); % broken option?...... bin plot cor vs umdist without separating quantdists
...
cortable = cortable(pars.include,:); % eliminate pairs that don't match criteria
...
switch pars.grouping_mode
    ...
    case 'quantile_groups'
        ...
        for ipair = 1:size(quant_group_pairs,1) %%% sort pairs by non-seedgroup quantgroup, excluding pairs with neither roi from the seedquant
            matches_seedgroup = quant_group_pairs(ipair,:) == seedgroup; % check which rois from this pair are from the seedgroup
            if nnz(matches_seedgroup) == 0 % if neither roi is from the seedgroup
                cortable.quantgroup_pair(ipair) = NaN; % do not plot
            elseif nnz(matches_seedgroup) == 1 % if one roi is from the seedgroup
                cortable.quantgroup_pair(ipair) = quant_group_pairs(ipair,~matches_seedgroup); % label this group by the roi NOT from the seedgroup
            elseif nnz(matches_seedgroup) == 2 % if both rois are from the seedgroup
                cortable.quantgroup_pair(ipair) = seedgroup; % label this pair with the seedgroup index
            end
        end
                
% % % % %         quant_group_pairs(matches_seedquant) = NaN; % demarcate the ROIs from within the seedgroup
% % % % %         cortable.quantgroup_pair = max(quant_group_pairs,[],2); % find the group of pair members that are NOT from the seedgroup
% % % % %         cortable.quantgroup_pair(isnan(cortable.quantgroup_pair)) = seedgroup; % mark ONLY pairs where both members are from the seedgroup with that group index
