%% this version assumes extra scope pulses at end of a session with multiple planes need to be deleted

%%%%% get_f_behavior_stim: align 2-photon scope frames with simultaneously collected locomotion
%%%%%   data and visual stimulus data
%%%%% last updated 2020/01/20  on thermaltake
...
if strcmp(stimpars.computer,'ANDREWLAB-PC') %% if from 4th floor scope, assume unidirectional acquisition
    ...
    if ~isempty(pars.all_scopetiming_files) && nplanes_in_session_total > 1 % if preceding sessions are listed and multiple planes were acquired
        event_rows_to_delete = []; 
        event_rows_within_a_session = []; % mark event rows within each session, warn if there are events found outside any session
        for isession = 1:length(pars.all_scopetiming_files)
            ...
            n_remainder_events = mod(length(scope_event_rows_this_session), nplanes_in_session_total);
            event_rows_to_delete = [event_rows_to_delete; scope_event_rows_this_session(end-n_remainder_events+1:end)]; 
            event_rows_within_a_session = [event_rows_within_a_session; scope_event_rows_this_session];
        end
        if height(scope_events_allplanes) > length(event_rows_within_a_session) %%% check that all scope events are accounted for by scopetiming files
        ...
        scope_events_allplanes(event_rows_to_delete,:) = []; % delete extra scope events without corresponding scope image
    end
    scope_events = scope_events_allplanes(iplane:nplanes_in_session_total:end,:);
end
...
nscopevents_orig = height(scope_events);
if nimages < nscopevents_orig % if scope images are missing
    if ~pars.skip_all_checks
        go_on = input(['Found fewer scope images (' num2str(nimages) ') than pulses (' num2str(nscopevents_orig) ') in scope channel - ignore extra pulses? (y=yes) '],'s');
        if ~strcmp(go_on,'y')
            error('quitting this function')
        end
    end
...
elseif nimages > nscopevents_orig % if scope trigger events are missing
    if ~pars.skip_all_checks
        go_on = input(['Found more scope images (' num2str(nimages) ') than pulses (' num2str(nscopevents_orig) ') in scope channel - ignore extra images? (y=yes) '],'s');
        if ~strcmp(go_on,'y')
            error('quitting this function')
        end
    end
...
for ievent = 1:nevents
    ...
    thisevent_tpoints = scope_events.onset(ievent) : end_tpoint;  % time points covered by this scope events
    % locomotion
    if strcmp(stimpars.computer,'IMAGING_VR-PC') %% if recorded on han lab scope
