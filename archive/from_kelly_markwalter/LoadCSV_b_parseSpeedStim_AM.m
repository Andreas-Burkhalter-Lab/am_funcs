...
% updated 2018-12-14 on thermaltake
...
%% Convert Encoder to RPM
    ...
    %Determine timepoints of rising signal (1); flat or down signal (0)
        encT(2:size(encT,1),3)=diff(encT(:,1))>0.1;  %Col3: Input 1 rising
        encT(2:size(encT,1),4)=diff(encT(:,2))>0.1;  %Col4: Input 2 rising
        encT(2:size(encT,1),5)=diff(encT(:,1))<-0.1;  %Col5: Input 1 falling
        encT(2:size(encT,1),6)=diff(encT(:,2))<-0.1;  %Col6: Input 2 falling
...
%     twotimes=timeT(jrow(twoloc),1);
    ...
    speedsT(:,3)=speedsT(:,2).*direction(:,7);
