function map=analyzeAveragedMapMHnew20100421_current_clamp(map,varargin) %%% AM 4/7/15 added varargin to allow for plot scale bar size in mV

%%%%%%% edited by AM 4/2/15 to not threshold amplitude values (set threshold arbitrarily high)
%%% %%% see line 49
...
if ~isempty(varargin)       % if plot scale bar was specified
    arrayTracesAsInputMapTTX_current_clamp(map,varargin) % remove TEST when done editing
else        % if plot scale bar unspecified - defaults to 50mV
    arrayTracesAsInputMapTTX_current_clamp(map)
end
