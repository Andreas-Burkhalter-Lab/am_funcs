...
if ~isempty(varargin)       % if plot scale bar was specified
    [handles selectedTrace] = arrayTracesAsInputMap_currentClamp_pickTraces(map,varargin);
else        % if plot scale bar unspecified - defaults to 50mV
    [handles selectedTrace] = arrayTracesAsInputMap_currentClamp_pickTraces(map);
end
